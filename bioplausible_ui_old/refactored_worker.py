...
from .utils import validate_hyperparams
...
class TrainingWorker(QThread):
    ...
    def __init__(
        ...
        # Validation
        if not validate_hyperparams({'lr': lr, 'epochs': epochs}):
            raise ValueError(f"Invalid hyperparameters: lr={lr}, epochs={epochs}")
    ...
    def _initialize_trainer(self) -> Optional[SupervisedTrainer]:
    ...
    def _train_epoch(self, epoch: int, trainer, num_batches: int, total_start: float) -> Optional[dict]:
    ...
    def _emit_batch_progress(self, epoch: int, batch_idx: int, num_batches: int, epoch_loss: float,
                           epoch_correct: int, epoch_total: int, x, batch_time: float, total_start: float,
                           batch_correct_last: int, batch_total_last: int):
    ...
    def _emit_epoch_metrics(self, epoch: int, num_batches: int, epoch_loss: float, epoch_correct: int,
                          epoch_total: int, lipschitz: float):
    ...
    def _generate_text(self, epoch: int):
