...
def benchmark_gpu(input_dim, hidden_dim, output_dim, n_samples, epochs, max_steps):
    ...
    kernel = EqPropKernelBPTT(
        input_dim, hidden_dim, output_dim, max_steps=max_steps, lr=0.01, use_gpu=use_gpu
    )
...
def main():
    ...
    if all_results:
        ...
        # Recommendation
        avg_speedup = np.mean(
            [r["comparison"]["speedup"] for r in all_results.values()]
        )
        avg_acc_gap = np.mean(
            [r["comparison"]["acc_gap"] for r in all_results.values()]
        )
