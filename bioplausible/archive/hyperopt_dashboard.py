    ...
    def update_visualizations(self):
        ...
        for model_name, trials in trials_by_model.items():
            ...
            self.plot_acc_speed.plot(
                times, accs, pen=None, symbol="o", symbolSize=6, symbolBrush=color
            )
            self.plot_acc_params.plot(
                params, accs, pen=None, symbol="o", symbolSize=6, symbolBrush=color
            )
            self.plot_speed_params.plot(
                params, times, pen=None, symbol="o", symbolSize=6, symbolBrush=color
            )
    ...
    def update_status_table(self):
        ...
        for i, trial in enumerate(recent_trials):
            ...
            self.status_table.setItem(
                i, 1, QTableWidgetItem(trial.model_name.split("(")[0].strip())
            )
            self.status_table.setItem(
                i, 2, QTableWidgetItem(f"{trial.epochs_completed}")
            )
    ...
    def export_configs(self):
        ...
        for model_name in self.model_names:
            ...
            if best_trials:
                ...
                filename = (
                    export_dir
                    / f"{model_name.replace(' ', '_').replace('(', '').replace(')', '')}.json"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter Optimization Dashboard"
    )
    parser.add_argument(
        "--task",
        type=str,
