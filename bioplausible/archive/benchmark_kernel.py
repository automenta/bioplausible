...
def benchmark_training(
    input_dim, hidden_dim, output_dim, n_samples, epochs, max_steps, use_sn=False
):
    ...
    torch_model = LoopedMLP(
        input_dim, hidden_dim, output_dim, use_spectral_norm=use_sn, max_steps=max_steps
    )
    ...
    with torch.no_grad():
        pt_train_acc = (
            (torch_model(X_train_torch).argmax(dim=1) == y_train_torch)
            .float()
            .mean()
            .item()
        )
        pt_test_acc = (
            (torch_model(X_test_torch).argmax(dim=1) == y_test_torch)
            .float()
            .mean()
            .item()
        )
    ...
    kernel = EqPropKernelBPTT(
        input_dim, hidden_dim, output_dim, max_steps=max_steps, lr=0.01
    )
...
def main():
    ...
    for scenario in scenarios:
        ...
        print(
            f"  {'Kernel':12} | Acc: {k['train_acc']*100:.1f}%/{k['test_acc']*100:.1f}% | "
            f"Time: {k['time']:.2f}s | Rate: {k['epochs_per_sec']:.1f} ep/s"
        )
        print(f"\n  Accuracy gap: {c['acc_gap']:.1f}%")
        print(
            f"  Speedup: {c['speedup']:.2f}× ({'KERNEL FASTER' if c['kernel_faster'] else 'PyTorch faster'})"
        )
    ...
    for name, results in all_results.items():
        ...
        print(
            f"| {name[:30]:30s} | {pt['test_acc']*100:.1f}% | {k['test_acc']*100:.1f}% | "
