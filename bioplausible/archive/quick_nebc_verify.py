...
def test_algorithm(name, model_class, X_train, y_train, X_test, y_test, **kwargs):
    ...
    for use_sn in [True, False]:
        ...
        # Train
        try:
            losses, train_accs = quick_train(
                model, X_train, y_train, epochs=20, lr=0.01
            )
        except Exception as e:
    ...
    if "with_sn" in results and "without_sn" in results:
        print(f"\n  [Comparison]")
        acc_delta = (
            results["with_sn"]["test_acc"] - results["without_sn"]["test_acc"]
        ) * 100
        ...
        print(
            f"    SN Stabilizes: {'✅' if results['with_sn']['lipschitz'] <= 1.1 else '❌'}"
        )
...
def main():
    ...
    for name, model_class, kwargs in algorithms:
        results = test_algorithm(
            name, model_class, X_train, y_train, X_test, y_test, **kwargs
        )
    ...
    print("=" * 60)
    print(
        f"{'Algorithm':<25} {'Acc (SN)':<12} {'Acc (No SN)':<12} {'Δ Acc':<10} {'L (SN)':<10} {'Verdict'}"
    )
    print("-" * 90)
    ...
    for name, results in all_results.items():
        if "with_sn" in results and "without_sn" in results:
            ...
            print(
                f"{name:<25} {acc_sn:>6.1f}%      {acc_no:>6.1f}%      {delta:>+5.1f}%    {L_sn:>6.3f}    {verdict}"
            )
    ...
    print(
        "- If 'L (SN)' > 1.1: SN not fully constraining (increase spectral norm power iterations?)"
    )
