...
class DepthPanel(QWidget):
    ...
    def __init__(self):
        ...
        self.cube_model = NeuralCube(cube_size=6, input_dim=64, output_dim=10).to(
            self.device
        )
    ...
    def init_ui(self):
        ...
        header.setStyleSheet(
            "padding: 10px; background-color: #3498db; border-radius: 5px;"
        )
    ...
    def create_depth_demo(self):
        ...
        self.depth_curve_sn = self.depth_plot.plot(
            name="With SN (L≤1)", pen=pg.mkPen("#27ae60", width=3)
        )
        self.depth_curve_no_sn = self.depth_plot.plot(
            name="Without SN (L>1)", pen=pg.mkPen("#e74c3c", width=3)
        )
    ...
    def create_gradient_demo(self):
        ...
        self.backprop_gradient_curve = self.backprop_gradient_plot.plot(
        ...
        self.eqprop_signal_curve = self.eqprop_signal_plot.plot(
            pen=pg.mkPen("#27ae60", width=3)
        )
    ...
    def run_depth_test(self):
        ...
        with torch.no_grad():
            ...
            if depth <= 500:
                # Use actual model
                _, norms_sn = self.deep_model.forward(
                    sample_x, return_signal_norms=True
                )
    ...
    def activate_cube(self):
        ...
        with torch.no_grad():
            sample_x = torch.randn(1, 64, device=self.device)
            _, trajectory = self.cube_model.forward(
                sample_x, steps=10, return_trajectory=True
            )
        ...
        self.cube_status.setStyleSheet(
            "color: #27ae60; font-weight: bold; font-size: 10px;"
        )
    ...
    def compare_gradients(self):
        ...
        self.gradient_status.setStyleSheet(
            "color: #27ae60; font-weight: bold; font-size: 10px;"
        )
