...
class NeuromorphicPanel(QWidget):
    ...
    def init_ui(self):
        ...
        header.setStyleSheet(
            "padding: 12px; background-color: #2c3e50; border-radius: 5px; border: 2px solid #9b59b6;"
        )
    ...
    def create_noise_demo(self):
        ...
        self.noise_curve_eqprop = self.noise_plot.plot(
            name="EqProp (L<1)", pen=pg.mkPen("#27ae60", width=3)
        )
        self.noise_curve_backprop = self.noise_plot.plot(
            name="Backprop (L>1)", pen=pg.mkPen("#e74c3c", width=3)
        )
