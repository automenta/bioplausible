...
class BiologicalPanel(QWidget):
    ...
    def __init__(self):
        ...
        self.eqprop_model = LoopedMLP(
            input_dim=64, hidden_dim=128, output_dim=10, use_spectral_norm=True
        ).to(self.device)
    ...
    def init_ui(self):
        ...
        header.setStyleSheet(
            "padding: 10px; background-color: #27ae60; border-radius: 5px;"
        )
    ...
    def create_healing_demo(self):
        ...
        self.healing_btn.setStyleSheet(
            "background-color: #e74c3c; color: white; padding: 8px; font-weight: bold;"
        )
        ...
        self.healing_status.setStyleSheet(
            "color: #27ae60; font-size: 10px; padding: 3px;"
        )
    ...
    def create_hebbian_demo(self):
        ...
        self.hebbian_curve_sn = self.hebbian_plot.plot(
            name="With SN", pen=pg.mkPen("#27ae60", width=2)
        )
        self.hebbian_curve_no_sn = self.hebbian_plot.plot(
            name="Without SN", pen=pg.mkPen("#e74c3c", width=2)
        )
        ...
        self.hebbian_btn.setStyleSheet(
        ...
        self.hebbian_status.setStyleSheet(
            "color: #27ae60; font-size: 10px; padding: 3px;"
        )
    ...
    def create_lipschitz_demo(self):
        ...
        threshold_text = pg.TextItem(
            "L = 1 (Threshold)", color="#e74c3c", anchor=(0, 1)
        )
        threshold_text.setPos(0.5, 1.0)
        self.lipschitz_bar.addItem(threshold_text)

        # Current value bar
        self.lipschitz_bar_item = pg.BarGraphItem(
            x=[0.5], height=[0], width=0.8, brush="#27ae60"
        )
    ...
    def run_healing(self):
        ...
        with torch.no_grad():
            ...
            result = self.eqprop_model.inject_noise_and_relax(
                sample_x, noise_level=1.0, injection_step=15, total_steps=30
            )
        ...
        self.healing_status.setText(
            f"Damping: {damping_pct:.1f}% reduction " f"({initial:.3f} → {final:.3f})"
        )
        self.healing_status.setStyleSheet(
            "color: #27ae60; font-weight: bold; font-size: 10px;"
        )
    ...
    def run_hebbian(self):
        ...
        norms_no_sn = np.array(norms_no_sn) * (
            0.8 + 0.4 * np.random.rand(len(norms_no_sn))
        )
