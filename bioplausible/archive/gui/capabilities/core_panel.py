...
class CorePanel(QWidget):
    ...
    def init_ui(self):
        ...
        header.setStyleSheet(
            "padding: 12px; background-color: #2c3e50; border-radius: 5px; border: 2px solid #3498db;"
        )
        ...
        self.free_curve = self.energy_plot.plot(
            name="Free Phase", pen=pg.mkPen("#3498db", width=2)
        )
        self.nudged_curve = self.energy_plot.plot(
            name="Nudged Phase", pen=pg.mkPen("#e74c3c", width=2)
        )
    ...
    def update_animation(self):
        ...
        with torch.no_grad():
            ...
            if self.current_step == 0:
                ...
                self.h_nudged = torch.zeros(
                    1, self.model.hidden_dim, device=self.device
                )
            ...
            self.h_nudged = torch.tanh(
                self.x_proj + self.model.W_rec(self.h_nudged) + hidden_nudge.t()
            )
            ...
            energy_free = (
                (
                    (
                        self.h_free
                        - torch.tanh(self.x_proj + self.model.W_rec(self.h_free))
                    )
                    ** 2
                )
                .sum()
            ...
            energy_nudged = (
                (
                    (
                        self.h_nudged
                        - torch.tanh(self.x_proj + self.model.W_rec(self.h_nudged))
                    )
                    ** 2
                )
                .sum()
            ...
            self.free_trajectory.append(
                {"energy": energy_free, "hidden": self.h_free.cpu().numpy()[0]}
            )
            self.nudged_trajectory.append(
                {"energy": energy_nudged, "hidden": self.h_nudged.cpu().numpy()[0]}
            )
