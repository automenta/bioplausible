...
class SimpleLM(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_dim=128):
        ...
        self.tf = nn.TransformerEncoderLayer(
            hidden_dim, 4, hidden_dim * 2, batch_first=True, dropout=0.1
        )
    ...
    def _create_model(self):
        ...
        if model_type == "backprop":
            ...
            return create_model(
                ...
                * min(self.num_layers, 5),  # Use up to 5 layers
    ...
    def train_step(self, x, y, step_num) -> TrainingState:
        ...
        try:
            if self.has_embed:
                ...
                ):  # Custom algorithm with train_step
                    out = (
                        self.model(h, steps=self.steps)
                        if hasattr(self.model, "W_rec")
                        else self.model(h)
                    )
                else:
                    out = self.model(h)
            ...
            if hasattr(self.model, "layers"):
                ...
                norms = [
                    self.model.W_in.weight.norm().item(),
                    self.model.W_rec.weight.norm().item(),
                    self.model.W_out.weight.norm().item(),
                ]
            ...
            try:
                for _ in range(length):
                    if self.has_embed:
                        ...
                        if hasattr(self.model, "train_step"):  # Custom algorithm
                            logits = (
                                self.model(ctx, steps=self.steps)
                                if hasattr(self.model, "eq_steps")
                                else self.model(ctx)
                            )
                        else:
                    ...
                    curr = torch.cat(
                        [curr, torch.tensor([next_tok], device=self.device)]
                    )
            except:
                pass
