...
class HealingDemo(QWidget):
    ...
    def init_ui(self):
        ...
        header.setStyleSheet(
            "padding: 10px; background-color: #2c3e50; border-radius: 5px;"
        )
        ...
        self.clean_curve_sn = self.sn_plot.plot(
            name="Clean", pen=pg.mkPen("#3498db", width=2)
        )
        self.noisy_curve_sn = self.sn_plot.plot(
            name="Noisy (injected @ 15)",
            pen=pg.mkPen("#e74c3c", width=2, style=Qt.PenStyle.DashLine),
        )
        self.sn_plot.addLegend()

        # Add injection marker
        self.injection_line_sn = pg.InfiniteLine(
            pos=15,
            angle=90,
            pen=pg.mkPen("#f39c12", width=1, style=Qt.PenStyle.DashLine),
        )
        ...
        self.no_sn_curve = self.no_sn_plot.plot(
            name="With noise", pen=pg.mkPen("#e74c3c", width=2)
        )
        self.no_sn_plot.addLegend()

        # Add injection marker
        self.injection_line_no_sn = pg.InfiniteLine(
            pos=15,
            angle=90,
            pen=pg.mkPen("#f39c12", width=1, style=Qt.PenStyle.DashLine),
        )
    ...
    def train_models(self):
        ...
        for epoch in range(2):
            loss_sn, acc_sn = train_quick_epoch(
                self.model_sn, train_loader, optimizer_sn, self.device
            )

        # Train without SN
        optimizer_no_sn = optim.Adam(self.model_no_sn.parameters(), lr=0.001)
        for epoch in range(2):
            loss_no_sn, acc_no_sn = train_quick_epoch(
                self.model_no_sn, train_loader, optimizer_no_sn, self.device
            )

        self.trained = True
        self.run_btn.setEnabled(True)
        self.status_label.setText(
            f"✓ Training complete! With SN: {acc_sn*100:.1f}%, Without SN: {acc_no_sn*100:.1f}%"
        )
    ...
    def update_animation(self):
        ...
        if self.current_step >= len(self.clean_trajectory):
            ...
            damping = (
                abs(clean_final - noisy_final)
                / abs(self.noisy_trajectory[15] - self.clean_trajectory[15])
                * 100
            )
    ...
    def reset(self):
        ...
        self.run_btn.setText("2️⃣ Run Healing Demo")
        self.status_label.setText(
            "Click 'Train Models' to begin"
            if not self.trained
            else "Click 'Run Healing Demo'"
        )
