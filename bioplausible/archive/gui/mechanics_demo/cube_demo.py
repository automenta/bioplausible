...
class CubeDemo(QWidget):
    ...
    def __init__(self):
        ...
        self.model = NeuralCube(
            cube_size=6, input_dim=784, output_dim=10, max_steps=15
        ).to(self.device)
    ...
    def init_ui(self):
        ...
        try:
            ...
            self.scatter = gl.GLScatterPlotItem(
                pos=self.positions, color=(0.5, 0.5, 0.5, 0.8), size=0.3
            )
        ...
        stats_label.setStyleSheet(
            "padding: 10px; background-color: #34495e; border-radius: 5px;"
        )
    ...
    def train_cube(self):
        ...
        for epoch in range(2):
            loss, acc = train_quick_epoch(
                self.model, train_loader, optimizer, self.device
            )
            self.status_label.setText(
                f"Epoch {epoch+1}/2: Loss={loss:.3f}, Acc={acc*100:.1f}%"
            )
    ...
    def run_activation_flow(self):
        ...
        with torch.no_grad():
            _, trajectory = self.model.forward(
                digit_img, steps=15, return_trajectory=True
            )
    ...
    def update_animation(self):
        ...
        if self.use_3d:
            ...
            act_norm = (activations - activations.min()) / (
            ...
            slice_2d = activations.reshape(
                self.cube_size, self.cube_size, self.cube_size
            )[z_mid]
            self.slice_image.setImage(slice_2d)

        self.status_label.setText(
            f"Convergence step {self.current_step+1}/15 - Watch activations flow!"
        )
        self.current_step += 1
