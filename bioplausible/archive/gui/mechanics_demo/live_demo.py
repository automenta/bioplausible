...
class LiveMechanicsDemo(QWidget):
    ...
    def init_ui(self):
        ...
        header.setStyleSheet(
            "padding: 10px; background-color: #1a1a2e; border-radius: 5px;"
        )
        ...
        self.start_btn.setStyleSheet(
            "background-color: #27ae60; color: white; padding: 15px; font-size: 16px; font-weight: bold;"
        )
        ...
        self.lip_plot.addLine(
            y=1.0, pen=pg.mkPen("#e74c3c", width=2, style=Qt.PenStyle.DashLine)
        )  # L=1 threshold
    ...
    def reset_model(self):
        ...
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.lr_spin.value())

        # Load data
        self.train_loader = load_mnist_subset(
            train=True, subset_size=1000, batch_size=32
        )
    ...
    def toggle_training(self):
        ...
        if self.training:
            ...
            self.start_btn.setStyleSheet(
                "background-color: #e74c3c; color: white; padding: 15px; font-size: 16px; font-weight: bold;"
            )
        ...
        with torch.no_grad():
            ...
            if len(trajectory) > 1:
                ...
                for i in range(1, len(trajectory)):
                    delta = (
                        (trajectory[i] - trajectory[i - 1]).norm(dim=1).mean().item()
                    )
