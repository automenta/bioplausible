...
class TrainingDemo(QWidget):
    ...
    def init_ui(self):
        ...
        self.eqprop_loss_curve = self.loss_plot.plot(
            name="EqProp", pen=pg.mkPen("#3498db", width=2)
        )
        self.backprop_loss_curve = self.loss_plot.plot(
            name="Backprop", pen=pg.mkPen("#e74c3c", width=2)
        )
        ...
        self.eqprop_acc_curve = self.acc_plot.plot(
            name="EqProp", pen=pg.mkPen("#3498db", width=2)
        )
        self.backprop_acc_curve = self.acc_plot.plot(
            name="Backprop", pen=pg.mkPen("#e74c3c", width=2)
        )
        ...
        self.status_label = QLabel(
            f"Click 'Start Training' to begin {self.max_epochs}-epoch comparison"
        )
    ...
    def start_training(self):
        ...
        # Data
        self.train_loader = load_mnist_subset(
            train=True, subset_size=1000, batch_size=64
        )
    ...
    def train_one_epoch(self):
        ...
        eqprop_loss, eqprop_acc = train_quick_epoch(
            self.eqprop_model, self.train_loader, self.eqprop_optimizer, self.device
        )

        # Train Backprop
        backprop_loss, backprop_acc = train_quick_epoch(
            self.backprop_model, self.train_loader, self.backprop_optimizer, self.device
        )
        ...
        with torch.no_grad():
            ...
            try:
                ...
                self.activation_image.setImage(
                    activations.T
