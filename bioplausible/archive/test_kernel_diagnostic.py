...
def test_architecture_match():
    ...
    torch_model = LoopedMLP(
        input_dim, hidden_dim, output_dim, use_spectral_norm=False, max_steps=30
    )
...
def test_training_convergence():
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(
                f"  Epoch {epoch+1:3d}: loss={result['loss']:.4f}, acc={result['accuracy']*100:.1f}%"
            )
    ...
    print(
        f"Kernel final:  loss={kernel_losses[-1]:.4f}, acc={kernel_accs[-1]*100:.1f}%"
    )
...
def test_gradient_correctness():
    ...
    torch_model = LoopedMLP(
        input_dim, hidden_dim, output_dim, use_spectral_norm=False, max_steps=5
    )
    ...
    with torch.no_grad():
        ...
        kernel.W_out = torch_model.W_out.weight.detach().cpu().numpy().copy()
        kernel.b_in = (
            torch_model.W_in.bias.detach().cpu().numpy().copy()
            if torch_model.W_in.bias is not None
            else np.zeros(hidden_dim, dtype=np.float32)
        )
        kernel.b_out = (
            torch_model.W_out.bias.detach().cpu().numpy().copy()
            if torch_model.W_out.bias is not None
