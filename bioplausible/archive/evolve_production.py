...
class DataCollector:
    ...
    def record(
        ...
        record = EvaluationRecord(
            ...
            fitness={
                ...
                "perplexity": (
                    fitness.perplexity if fitness.perplexity < float("inf") else -1
                ),
                "speed": fitness.speed,
                "memory_mb": fitness.memory_mb,
                "lipschitz": (
                    fitness.lipschitz if fitness.lipschitz < float("inf") else -1
                ),
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for mtype, recs in by_type.items():
            successful = [
                r for r in recs if r["success"] and r["fitness"]["accuracy"] > 0
            ]
            if successful:
...
class DiversePopulation:
    ...
    def initialize(self):
        ...
        self.logger.info(
            f"Initialized population with {len(self.individuals)} individuals"
        )
        self._log_diversity()
    ...
    def evolve_next_generation(self, generation: int):
        ...
        for ind in evaluated:
            ...
            if len(new_pop) >= int(
                self.config.population_size * self.config.elite_fraction
            ):
                break
        ...
        while len(new_pop) < self.config.population_size:
            ...
            type_parents = [
                ind for ind in evaluated if ind["arch_config"].model_type == target_type
            ]

            if (
                len(type_parents) >= 2
                and np.random.random() < self.config.crossover_rate
            ):
                p1, p2 = np.random.choice(type_parents, size=2, replace=False)
                child_arch = self.arch_breeder.crossover(
                    p1["arch_config"], p2["arch_config"]
                )
                if (
                    self.config.explore_algorithms
                    and p1["algo_config"]
                    and p2["algo_config"]
                ):
                    child_algo = self.algo_breeder.crossover(
                        p1["algo_config"], p2["algo_config"]
                    )
                else:
                    child_algo = None
            elif type_parents:
                ...
                child_algo = (
                    parent["algo_config"].clone() if parent["algo_config"] else None
                )
            else:
                # Generate new random
                ...
                child_algo = (
                    self.algo_breeder.generate_informed()
                    if self.config.explore_algorithms
                    else None
                )
            ...
            "individuals": [
                {
                    ...
                    "algo_config": (
                        ind["algo_config"].to_dict() if ind["algo_config"] else None
                    ),
    ...
    def from_dict(self, data: Dict[str, Any]):
        ...
        for d in data["individuals"]:
            ind = {
                ...
                "algo_config": (
                    AlgorithmConfig.from_dict(d["algo_config"])
                    if d["algo_config"]
                    else None
                ),
...
class ProductionRunner:
        ...
        try:
            while time.time() < end_time:
                ...
                if (
                    ...
                    > self.config.checkpoint_interval_min * 60
                ):
                ...
                self.logger.info(
                    f"Generation time: {gen_time:.1f}s, Remaining: {remaining/3600:.1f}h"
                )
    ...
    def _evaluate_population(self):
        ...
        for i, ind in enumerate(unevaluated):
            ...
            self.logger.info(
                f"  [{i+1}/{len(unevaluated)}] Evaluating {arch.model_type} "
            ...
            try:
                ...
                self.logger.info(
                    f"    Acc: {fitness.accuracy:.4f}, L: {fitness.lipschitz:.3f}"
                )
    ...
    def _save_checkpoint(self):
        ...
        self.logger.info(
            f"Checkpoint saved: gen={self.generation}, evals={self.total_evaluations}"
        )
    ...
    def _generate_report(self):
        ...
        successful = [
            r for r in records if r["success"] and r["fitness"]["accuracy"] > 0
        ]
        top_10 = sorted(
            successful, key=lambda r: r["fitness"]["accuracy"], reverse=True
        )[:10]

        for i, rec in enumerate(top_10, 1):
            lines.append(
                f"### #{i}: {rec['model_type']} - {rec['fitness']['accuracy']:.4f}"
            )
            lines.append("```json")
...
def parse_args():
    ...
    mode.add_argument(
        "--resume", type=str, metavar="DIR", help="Resume from checkpoint in directory"
    )
    mode.add_argument(
        "--report", type=str, metavar="DIR", help="Generate report from existing data"
    )
    mode.add_argument(
        "--continue",
        dest="continue_dir",
    ...
    # Duration
    parser.add_argument(
        "--hours", type=float, default=8.0, help="Maximum runtime in hours (default: 8)"
    )

    # Population
    parser.add_argument(
        "--population", "-p", type=int, default=30, help="Population size (default: 30)"
    )
    ...
    # Output
    parser.add_argument(
        "--output", "-o", type=str, default="results/evolution", help="Output directory"
    )

    # Advanced
    parser.add_argument(
        "--no-algorithms", action="store_true", help="Disable algorithm evolution"
    )
...
def main():
    ...
    if args.report:
        ...
        for mtype, mstats in stats.get("by_model_type", {}).items():
            print(
                f"  {mtype}: best={mstats['max_acc']:.4f}, "
