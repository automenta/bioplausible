...
def test_power_iterations():
    ...
    for n_iter in power_iters:
        ...
        print(
            f"  n_power_iterations={n_iter:2d}: L = {actual_L:.6f} {'✅' if actual_L <= 1.01 else '❌'}"
        )
    ...
    class ImprovedDFA(nn.Module):
        def __init__(self, input_dim=784, hidden_dim=256, output_dim=10, num_layers=3):
            ...
            self.W_in = spectral_norm(
                nn.Linear(input_dim, hidden_dim), n_power_iterations=5
            )
            ...
            self.head = spectral_norm(
                nn.Linear(hidden_dim, output_dim), n_power_iterations=5
            )
        ...
        def forward(self, x, steps=5):
            batch_size = x.size(0)
            h = [
                torch.zeros(batch_size, layer.out_features, device=x.device)
...
def test_hebbian_chain_depths():
    ...
    for use_sn in [True, False]:
        ...
        for depth in depths:
            ...
            print(
                f"    Depth {depth:3d}: decay={signal_info['decay_ratio']:.4f}, acc={acc*100:.1f}%"
            )
    ...
    for depth in depths:
        ...
        if without_sn < 0.01 and with_sn > 0.01:
            print(
                f"   ⚠️ Depth {depth}: Signal collapses without SN (decay: {without_sn:.4f} → {with_sn:.4f})"
            )
