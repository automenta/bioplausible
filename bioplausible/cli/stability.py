...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # report
    report_parser = subparsers.add_parser("report", help="Generate stability report for a run")
    report_parser.add_argument("--run-id", required=True, help="Run ID (campaign ID or episode ID)")
    ...
    # compare
    compare_parser = subparsers.add_parser("compare", help="Compare stability across coordinates")
    compare_parser.add_argument("--coordinates", nargs="+", required=True, help="6-D coordinates to compare")
    ...
    # summary
    summary_parser = subparsers.add_parser("summary", help="Summary stability statistics")
...
def _generate_text_report(run_id: str, data: list[dict]) -> str:
    ...
    for i, d in enumerate(data[:10]):
        coord_short = d["coordinate"][:38] + ".." if len(d["coordinate"]) > 40 else d["coordinate"]
        lines.append(
    ...
    if data:
        ...
        lyap_vals = [d["lyapunov_local"] for d in data if d["lyapunov_local"] is not None]
        settle_vals = [d["settling_time"] for d in data if d["settling_time"] is not None]
        basin_vals = [d["basin_stability"] for d in data if d["basin_stability"] is not None]

        lines.append(f"  Stability Score: mean={statistics.mean(scores):.3f}, stdev={statistics.stdev(scores) if len(scores) > 1 else 0:.3f}")
        if rho_vals:
            lines.append(f"  ρ(Jacobian): mean={statistics.mean(rho_vals):.3f}, min={min(rho_vals):.3f}, max={max(rho_vals):.3f}")
        if lyap_vals:
            lines.append(f"  Lyapunov: mean={statistics.mean(lyap_vals):.3f}, min={min(lyap_vals):.3f}, max={max(lyap_vals):.3f}")
        if settle_vals:
            lines.append(f"  Settling Time: mean={statistics.mean(settle_vals):.1f}, min={min(settle_vals):.1f}, max={max(settle_vals):.1f}")
        if basin_vals:
            lines.append(f"  Basin Stability: mean={statistics.mean(basin_vals):.3f}, min={min(basin_vals):.3f}, max={max(basin_vals):.3f}")
...
def _compare_stability(args) -> int:
    ...
    if args.format == "json":
        ...
        output += "-" * 100 + "\n"
        for d in all_data:
            coord_short = d["coordinate"][:48] + ".." if len(d["coordinate"]) > 50 else d["coordinate"]
...
def _summary_stability(args) -> int:
    ...
    print("=" * 60)
    print(f"Stability Score: mean={statistics.mean(scores):.3f}, stdev={statistics.stdev(scores) if len(scores) > 1 else 0:.3f}, min={min(scores):.3f}, max={max(scores):.3f}")
    if rho_vals:
        print(f"ρ(Jacobian): mean={statistics.mean(rho_vals):.3f}, min={min(rho_vals):.3f}, max={max(rho_vals):.3f}")
    if lyap_vals:
        print(f"Lyapunov: mean={statistics.mean(lyap_vals):.3f}, min={min(lyap_vals):.3f}, max={max(lyap_vals):.3f}")
    if settle_vals:
        print(f"Settling Time: mean={statistics.mean(settle_vals):.1f}, min={min(settle_vals):.1f}, max={max(settle_vals):.1f}")
    if basin_vals:
        print(f"Basin Stability: mean={statistics.mean(basin_vals):.3f}, min={min(basin_vals):.3f}, max={max(basin_vals):.3f}")
