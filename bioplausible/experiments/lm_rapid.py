...
def exp_ablation(device, output_dir):
    ...
    print(
        f"  → PPL: {bp_result['perplexity']:.2f}, Acc: {bp_result['accuracy']:.3f}, "
    ...
    with open(output_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["name", "perplexity", "accuracy", "loss", "params", "time"]
        )
        writer.writeheader()
    ...
    for r in results:
        ...
        print(
            f"{r['name']:40s} | {r['perplexity']:8.2f} | {r['accuracy']:6.3f} | "
...
def main():
    ...
    parser.add_argument(
        "--exp",
        choices=["ablation", "efficiency", "all"],
        default="ablation",
