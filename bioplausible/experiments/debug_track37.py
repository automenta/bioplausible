...
def run_comparison(config_name, config, device, seed):
    ...
    if bp_lr is None:
        raise ValueError("No LR specified for Backprop")

    bp_ppl, bp_history = train_and_eval(
        bp_model, train_data, val_data, vocab_size, {**config, "lr": bp_lr}, device
    )
    ...
    for variant in config.get("variants", ["full"]):
        ...
        if eq_lr is None:
            raise ValueError(f"No LR specified for EqProp {variant}")

        eq_ppl, eq_history = train_and_eval(
            eq_model, train_data, val_data, vocab_size, {**config, "lr": eq_lr}, device
        )
...
def main():
    ...
    for config_name, config in configs.items():
        ...
        print(
            f"  LR_BP: {config.get('lr_bp', config.get('lr'))}, "
        ...
        for seed in range(args.seeds):
            ...
            for name, data in results.items():
                if name != "backprop":
                    ...
                    print(
                        f"    {name}: {eq_ppl:.2f} vs BP {bp_ppl:.2f} = {ratio:.2f}× {marker}"
                    )
    ...
    for config_name, seed_results in all_results.items():
        ...
        for key in seed_results[0].keys():
            if key != "backprop":
                ...
                print(
                    f"  {key}: {avg:.2f} ± {max(ppls)-min(ppls):.2f} ({avg/avg_bp:.2f}× BP)"
                )
