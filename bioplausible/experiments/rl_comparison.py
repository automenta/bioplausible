...
def main():
    ...
    model_looped_bptt = LoopedMLP(
        input_dim, args.hidden, output_dim,
        max_steps=args.steps, gradient_method='bptt',
        use_spectral_norm=True
    )
    trainer_looped_bptt = RLTrainer(model_looped_bptt, args.env, args.device, args.lr, seed=args.seed)
    ...
    model_eq = LoopedMLP(
        input_dim, args.hidden, output_dim,
        max_steps=args.steps, gradient_method='equilibrium',
        use_spectral_norm=True
    )
    ...
    for name, res in results.items():
        max_r = max([np.mean(res['rewards'][i:i+50]) for i in range(0, len(res['rewards']), 50)])
