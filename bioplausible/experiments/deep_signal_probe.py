...
def measure_layer_signals(model, h_perturbed, x_input=None):
    ...
    with torch.no_grad():
        if x_input is not None:
            ...
            x_transformed = torch.zeros(batch_size, model.hidden_dim, device=h_perturbed.device)
...
def create_deep_model(depth: int, input_dim: int = 784, hidden_dim: int = 64, output_dim: int = 10, 
                     backend: str = 'auto', use_residual: bool = False):
    """
...
def inject_perturbation_at_layer(model, layer_idx: int, perturbation_strength: float = 0.1):
    """
    ...
    perturbation = torch.randn(batch_size, model.hidden_dim, device=device) * perturbation_strength
    return perturbation


def run_signal_propagation_experiment(depths: List[int] = [10, 100, 500, 1000], 
...
def compare_with_skip_connections(depths: List[int] = [10, 100, 500, 1000], 
                                perturbation_strength: float = 0.1):
    """
    ...
    comparison_results = {
        ...
        'skip_connections': results_skip
    }
    
    return comparison_results


def visualize_signal_propagation(results: Dict, title: str = "Signal Propagation Analysis"):
    """
    ...
    try:
        ...
        plt.subplot(2, 2, 2)
        final_signals = [results['signals'][d][-1] if results['signals'][d] else 0 for d in depths]
...
def run_complete_signal_probe():
    ...
    for backend in backends_to_test:
        ...
        for depth in depths:
            final_signal = results['signals'][depth][-1] if results['signals'][depth] else 0.0
            time_taken = results['times'][depth]
            print(f"  Depth {depth:3d}: Final signal = {final_signal:.6f}, Time = {time_taken:.2f}s")
    
    # Visualize results for the primary backend
    if 'pytorch' in all_results:
        visualize_signal_propagation(
            all_results['pytorch'], 
            f"Signal Propagation - PyTorch Backend"
        )
    
    if 'kernel' in all_results:
        visualize_signal_propagation(
            all_results['kernel'], 
            f"Signal Propagation - Kernel Backend (O(1) Memory)"
        )
    ...
    for backend, results in all_results.items():
        if 1000 in results['depths']:  # If we tested depth 1000
            final_signal = results['signals'][1000][-1] if results['signals'][1000] else 0.0
            success = final_signal > 0.01  # > 1%
            print(f"{backend.capitalize()} backend signal at depth 1000: {final_signal:.6f} ({'SUCCESS' if success else 'FAILED'})")
