...
def run_benchmark(model_cls, name, epochs=3, **model_kwargs):
    ...
    # Create Data
    train_loader, test_loader = create_data_loaders('mnist', batch_size=128, flatten=True)
    input_dim = 784
    output_dim = 10
    ...
    if name == 'LoopedMLP (BPTT)':
        ...
        model = model_cls(input_dim=input_dim, output_dim=output_dim, hidden_dim=model_kwargs.get('hidden_dim', 256))
    else:
        # StandardEqProp/FA
        model = model_cls(input_dim=input_dim, output_dim=output_dim, hidden_dims=[model_kwargs.get('hidden_dim', 256)], **model_kwargs)
    ...
    trainer = EqPropTrainer(model, optimizer='adam', lr=0.001, use_kernel=False, use_compile=False, device=device)

    # Train
    start_time = time.time()
    history = trainer.fit(train_loader, epochs=epochs, val_loader=test_loader, log_interval=100)
    end_time = time.time()
...
def main():
    ...
    results.append(run_benchmark(StandardEqProp, "Equilibrium Propagation", hidden_dim=256, beta=0.5, equilibrium_steps=20))
    ...
    for res in results:
        ...
        print(f"{res['name']:<25} | {res['accuracy']:.2%}   | {res['duration']:.2f}s   | {res['duration']/epochs:.2f}")
