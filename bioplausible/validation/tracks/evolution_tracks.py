...
def track_60_evolution_validation(verifier) -> Dict[str, Any]:
    ...
    print(
        f"    Random: {results['random']['mean']:.4f} ± {results['random']['std']:.4f}"
    )
    ...
    evo_scores = [ind.fitness.accuracy for ind in state.population if ind.fitness]
    ...
    print(
        f"    Evolution: {results['evolution']['mean']:.4f} ± {results['evolution']['std']:.4f}"
    )
    ...
    cohens_d = (results["evolution"]["mean"] - results["random"]["mean"]) / max(
        pooled_std, 1e-6
    )
