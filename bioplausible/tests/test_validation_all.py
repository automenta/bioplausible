...
class TestValidationAll(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.y_conv = torch.randint(0, 10, (self.sample_size,)).to(self.device) # Models default to 10 classes
        self.loader_conv = DataLoader(TensorDataset(self.x_conv, self.y_conv), batch_size=self.batch_size, shuffle=True)

        # 3. Sequence Data for LMs (B, T) -> (B, T)
        self.x_seq = torch.randint(0, self.vocab_size, (self.sample_size, self.seq_len)).to(self.device)
        self.y_seq = torch.randint(0, self.vocab_size, (self.sample_size, self.seq_len)).to(self.device)
        self.loader_seq = DataLoader(TensorDataset(self.x_seq, self.y_seq), batch_size=self.batch_size, shuffle=True)

        # 4. Transformer Classification Data (B, T) -> (B,)
        # TransformerEqProp takes sequence input but pools to classification output
        self.y_trans_cls = torch.randint(0, self.output_dim, (self.sample_size,)).to(self.device)
        self.loader_trans_cls = DataLoader(TensorDataset(self.x_seq, self.y_trans_cls), batch_size=self.batch_size, shuffle=True)
        ...
        for epoch in range(self.epochs):
            ...
            for bx, by in loader:
                ...
                if hasattr(model, 'train_step') and not isinstance(model, (LoopedMLP)):
                     ...
                     total_loss += loss_val.item() if hasattr(loss_val, 'item') else loss_val
                     count += 1
                     continue
    ...
    def test_looped_mlp_learns(self):
        model = LoopedMLP(self.input_dim, 32, self.output_dim, max_steps=5).to(self.device)
    ...
    def test_looped_mlp_equilibrium_learns(self):
        model = LoopedMLP(self.input_dim, 32, self.output_dim, max_steps=5, gradient_method='equilibrium').to(self.device)
        i_loss, f_loss = self._train_minimal(model, self.loader)
    ...
    def test_transformer_eqprop_learns(self):
        ...
        model = TransformerEqProp(vocab_size=self.vocab_size, hidden_dim=16, output_dim=self.output_dim, num_layers=1, max_seq_len=10).to(self.device)
    ...
    def test_lm_models_learn(self):
         for cls in [FullEqPropLM, RecurrentEqPropLM]:
             model = cls(vocab_size=self.vocab_size, hidden_dim=16, num_layers=1, max_seq_len=10).to(self.device)
             i_loss, f_loss = self._train_minimal(model, self.loader_seq, is_sequence=True)
             print(f"{cls.__name__}: {i_loss:.4f} -> {f_loss:.4f}")
             self.assertLess(f_loss, i_loss)

    def test_adaptive_fa_learns(self):
        model = AdaptiveFA(self.input_dim, 32, self.output_dim, num_layers=2).to(self.device)
        i_loss, f_loss = self._train_minimal(model, self.loader)
        print(f"AdaptiveFA: {i_loss:.4f} -> {f_loss:.4f}")
        self.assertTrue(torch.isfinite(torch.tensor(f_loss)))

    def test_feedback_alignment_learns(self):
        model = FeedbackAlignmentEqProp(self.input_dim, 32, self.output_dim).to(self.device)
        i_loss, f_loss = self._train_minimal(model, self.loader)
