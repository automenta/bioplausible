...
class EvolutionaryOptimizer:
    ...
    def __init__(
        ...
        self.search_spaces = {name: get_search_space(name) for name in model_names}
    ...
    def select_parents(self, model_name: str) -> List[TrialMetrics]:
        ...
        for front_idx, front_indices in enumerate(fronts):
            ...
            for i, trial_idx in enumerate(front_indices):
                ...
                fitness_scores[trial_idx] = (len(fronts) - front_idx) + distances[
                    i
                ] / 10.0
        ...
        n_parents = max(
            2, int(self.config.population_size * self.config.elite_fraction)
        )
        return sorted_trials[:n_parents]

    def generate_offspring(
        self, model_name: str, parents: List[TrialMetrics], n_offspring: int
    ) -> List[int]:
        ...
        for _ in range(n_offspring):
            ...
            # Mutation
            child_config = space.mutate(
                child_config, self.config.mutation_rate, self.rng
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {"generation": self.generation, "models": {}}
        ...
        for model_name in self.model_names:
            ...
            stats["models"][model_name] = {
                ...
                "pareto_size": (
                    len(self.storage.get_all_trials(model_name)) if completed else 0
                ),
            }
