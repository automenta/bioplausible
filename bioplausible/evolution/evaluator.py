...
TIER_CONFIGS = {
    EvalTier.TIER_1_SMOKE: TierConfig(
        n_samples=500, epochs=3, n_seeds=1, timeout_sec=30
    ),
    EvalTier.TIER_2_QUICK: TierConfig(
        n_samples=5000, epochs=10, n_seeds=1, timeout_sec=300
    ),
    EvalTier.TIER_3_FULL: TierConfig(
        n_samples=60000, epochs=30, n_seeds=3, timeout_sec=1800
    ),
    EvalTier.TIER_4_BREAKTHROUGH: TierConfig(
        n_samples=50000, epochs=100, n_seeds=5, timeout_sec=7200
    ),
}
...
class VariationEvaluator:
    ...
    def evaluate(
        ...
        if self.verbose:
            print(
                f"[Tier {tier}] Evaluating {config.model_type} "
    ...
    def _aggregate_results(
        ...
        return FitnessScore(
            ...
            lipschitz=np.mean(
                [r.lipschitz for r in results if r.lipschitz < float("inf")]
            )
            or float("inf"),
    ...
    def _get_data(
        ...
        if task in ["mnist", "fashion"]:
            ...
            # Subset
            train_subset = Subset(
                train_data, list(range(min(n_samples, len(train_data))))
            )
