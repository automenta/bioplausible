...
class BreakthroughDetector:
    ...
    def get_breakthrough_dims(
        ...
        if (
            baseline.perplexity < float("inf")
            and result.perplexity < baseline.perplexity
        ):
    ...
    def generate_summary(self) -> str:
        ...
        sorted_reports = sorted(
            self.breakthroughs, key=lambda r: r.fitness.composite_score(), reverse=True
        )

        for report in sorted_reports[:10]:
            improvements = ", ".join(
                [f"{dim}: +{pct:.1f}%" for dim, pct in report.improvement_pct.items()]
            )
            lines.append(
