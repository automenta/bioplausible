...
class EvolutionEngine:
    ...
    def run(
        ...
        if verbose:
            print(
                f"Starting evolution: {n_gens} generations, pop={self.config.population_size}"
            )
    ...
    def _evaluate_population(self, verbose: bool = True) -> None:
        ...
        for i, ind in enumerate(self.state.population):
            ...
            if verbose:
                print(
                    f"  [{i+1}/{len(self.state.population)}] Evaluating {ind.config.model_type} "
            ...
            try:
                ...
                if ind.fitness.composite_score() > self.state.best_fitness:
                    ...
                    if verbose:
                        print(
                            f"    ★ New best: acc={ind.fitness.accuracy:.4f}, "
            ...
            stats.update(
                {
                    ...
                    "mean_lipschitz": np.mean(
                        [f.lipschitz for f in fitnesses if f.lipschitz < float("inf")]
                    ),
                }
            )
    ...
    def _print_generation_stats(self, stats: Dict[str, Any]) -> None:
        ...
        if "best_accuracy" in stats:
            ...
            print(
                f"    Mean Acc: {stats['mean_accuracy']:.4f} ± {stats['std_accuracy']:.4f}"
            )
    ...
    def _save_checkpoint(self) -> None:
        ...
        checkpoint_path = (
            self.output_dir / f"checkpoint_gen{self.state.generation}.json"
        )
