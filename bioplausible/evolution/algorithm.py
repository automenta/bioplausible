...
class UpdateRule(Enum):
    ...
    CONTRASTIVE_HEBBIAN = (
        "contrastive_hebbian"  # Classic: Δw ∝ h_nudged⊗h_nudged - h_free⊗h_free
    )
    SYMMETRIC_DIFF = (
        "symmetric_diff"  # Δw ∝ (h_nudged - h_free) ⊗ (pre_nudged - pre_free)
    )
...
class AlgorithmBreeder:
    ...
    def mutate(self, config: AlgorithmConfig) -> AlgorithmConfig:
        ...
        if self.rng.random() < self.mutation_rate * 0.5:
            child.update_rule = self.rng.choice(
                [r for r in self.UPDATE_RULES if r != child.update_rule]
            )
            child.mutations_applied.append(f"update_rule→{child.update_rule.value}")

        if self.rng.random() < self.mutation_rate * 0.5:
            child.equilibrium_dynamics = self.rng.choice(
                [d for d in self.DYNAMICS if d != child.equilibrium_dynamics]
            )
            child.mutations_applied.append(
                f"dynamics→{child.equilibrium_dynamics.value}"
            )

        if self.rng.random() < self.mutation_rate * 0.5:
            child.gradient_approx = self.rng.choice(
                [g for g in self.GRADIENT_APPROX if g != child.gradient_approx]
            )
        ...
        if self.rng.random() < self.mutation_rate:
            child.eq_steps = int(
                np.clip(child.eq_steps + self.rng.integers(-10, 11), 5, 100)
            )
            child.mutations_applied.append(f"eq_steps→{child.eq_steps}")

        if self.rng.random() < self.mutation_rate:
            child.beta = float(
                np.clip(child.beta + self.rng.normal(0, 0.05), 0.01, 0.5)
            )
            child.mutations_applied.append(f"beta→{child.beta:.3f}")

        if self.rng.random() < self.mutation_rate:
            child.alpha = float(
                np.clip(child.alpha + self.rng.normal(0, 0.1), 0.1, 0.95)
            )
        ...
        if self.rng.random() < self.mutation_rate * 0.3:
            ...
            child.mutations_applied.append(
                f"homeostatic→{child.use_homeostatic_plasticity}"
            )

        if self.rng.random() < self.mutation_rate * 0.3:
            child.use_lateral_inhibition = not child.use_lateral_inhibition
            child.mutations_applied.append(
                f"lateral_inhib→{child.use_lateral_inhibition}"
            )
        ...
        if child.equilibrium_dynamics == EquilibriumDynamics.MOMENTUM:
            if self.rng.random() < self.mutation_rate:
                child.momentum = float(
                    np.clip(child.momentum + self.rng.normal(0, 0.1), 0, 0.99)
                )
        ...
        if child.equilibrium_dynamics == EquilibriumDynamics.DAMPED_OSCILLATION:
            if self.rng.random() < self.mutation_rate:
                child.damping = float(
                    np.clip(child.damping + self.rng.normal(0, 0.05), 0.5, 0.99)
                )
    ...
    def crossover(self, p1: AlgorithmConfig, p2: AlgorithmConfig) -> AlgorithmConfig:
        ...
        child.equilibrium_dynamics = self.rng.choice(
            [p1.equilibrium_dynamics, p2.equilibrium_dynamics]
        )
        child.gradient_approx = self.rng.choice(
            [p1.gradient_approx, p2.gradient_approx]
        )
        ...
        child.use_homeostatic_plasticity = self.rng.choice(
            [p1.use_homeostatic_plasticity, p2.use_homeostatic_plasticity]
        )
        child.use_lateral_inhibition = self.rng.choice(
            [p1.use_lateral_inhibition, p2.use_lateral_inhibition]
        )
    ...
    def generate_random(self) -> AlgorithmConfig:
        ...
        return AlgorithmConfig(
            ...
            momentum=(
                float(self.rng.uniform(0, 0.9)) if self.rng.random() > 0.5 else 0.0
            ),
