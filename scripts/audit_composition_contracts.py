...
def make_joint_system(device: torch.device, plasticity_type: str = "fast_weights") -> ContinualJointSystem:
    ...
    if plasticity_type == "fast_weights":
        ...
        plasticity = RuleStatePlasticity(num_operators=8, operator_dim=64, device=device)
...
def test_system_context_construction() -> AuditTest:
    ...
    type_checks = {
        ...
        "dynamics_config_type": isinstance(context.dynamics_config, StateDynamicsConfig),
    ...
    return AuditTest(
        test="system_context_construction",
        passed=passed,
        details=all_checks
    )
...
def test_composite_state_structure() -> AuditTest:
    ...
    return AuditTest(
        test="composite_state_structure",
        passed=passed,
        details=all_checks
    )
...
def test_parameter_update_application() -> AuditTest:
    ...
    pseudo_grads = [torch.randn_like(geometry.params[name]) * 0.01 for name in learnable_names]
...
def test_device_propagation() -> AuditTest:
    ...
    geometry_params_ok = all(
        ...
        for p in continual.geometry.parameters()
    )
    ...
    if hasattr(continual.plasticity, "_proj_matrices"):
        ...
        plasticity_ok = (
            continual.plasticity._operator_embeddings.device.type == device_cuda.type and
            all(p.device.type == device_cuda.type for p in continual.plasticity._controller.parameters())
        )
...
def test_state_registry_integrity() -> AuditTest:
    ...
    checks["consolidatable_subset_of_fast_plastic"] = consolidatable_names.issubset(fast_plastic_names)
...
def test_all_plasticity_types() -> AuditTest:
    ...
    for plasticity_type in ["fast_weights", "routing", "rule_state", "null"]:
        try:
            ...
            ctx_ok = (
                context.theta is not None and
                context.geometry is not None and
                context.substrate is not None and
            ...
            expected_fast = set(continual.plasticity.config.plastic_state_dims.keys()) if plasticity_type != "null" else set()
