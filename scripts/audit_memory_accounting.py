...
def test_resourceusage_peak_activation_bytes() -> Dict[str, Any]:
    ...
    if not hasattr(usage, 'peak_activation_bytes'):
        ...
        print(f"  PASS: peak_activation_bytes field exists: {usage.peak_activation_bytes}")
    
    # Check other key fields
    required_fields = ['compute', 'memory', 'energy', 'latency', 'plastic_state_capacity',
                       'param_count', 'forward_flops', 'backward_flops']
    ...
    if all_passed:
        ...
        print(f"    compute: {usage.compute}, memory: {usage.memory}, peak_activation_bytes: {usage.peak_activation_bytes}")
    ...
    return {
        ...
        "serialization_works": usage.peak_activation_bytes == usage2.peak_activation_bytes,
    }
...
def test_gradient_checkpointing_peak() -> Dict[str, Any]:
    ...
    return {
        ...
        "resource_usage_matches": ru.peak_activation_bytes == gc_model.peak_activation_bytes,
    }
...
def test_plastic_state_bytes() -> Dict[str, Any]:
    ...
    if model._psi is not None and "fast_weights" in model._psi:
        actual_bytes = model._psi["fast_weights"].numel() * model._psi["fast_weights"].element_size()
    ...
    return {
        ...
        "actual_bytes": actual_bytes if model._psi is not None and "fast_weights" in model._psi else 0,
...
def test_replay_buffer_bytes() -> Dict[str, Any]:
    ...
    print(f"  Full buffer - Expected: {expected_bytes_full}, Actual: {actual_bytes_full}")
...
def test_memory_accounted_model_hooks() -> Dict[str, Any]:
    ...
    if hook_count < target_layers:
        all_passed = False
        print(f"  FAIL: Not all target layers have hooks ({hook_count} < {target_layers})")
    else:
    ...
    if peak_act_bytes == 0:
        ...
        print(f"  PASS: Peak activation captured: {peak_act_bytes / 1024 / 1024:.2f} MB")
