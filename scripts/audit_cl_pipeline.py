...
from computronium.core.continual.training import run_continual_train_step, _lwf_train_step, _si_train_step
from computronium.core.continual.constants import CL_CLASSES_PER_TASK, CL_NUM_TASKS
from computronium.core.continual.stability import create_stability_guard, make_transition_fn, check_stability
...
def test_task_masking() -> Dict[str, Any]:
    ...
    y_task1 = torch.tensor([0, 1, 0, 1], device=device)  # Task 1 labels (classes 2,3 -> local 0,1)
    ...
    passed = True
    print(f"Task 0 logits shape: {logits0.shape}, Task 0 loss: {loss0_direct.item():.4f}")
    print(f"Task 1 logits shape: {logits1.shape}, Task 1 loss: {loss1_direct.item():.4f}")
...
def test_replay_buffer() -> Dict[str, Any]:
    ...
    if rx.shape != (16, 784) or ry.shape != (16,) or rt.shape != (16,):
        ...
        print(f"  PASS: Sample shapes correct: x={rx.shape}, y={ry.shape}, t={rt.shape}")
    ...
    if rx2.shape[0] != capacity:
        all_passed = False
        print(f"  FAIL: Sampling more than capacity should return {capacity} samples, got {rx2.shape[0]}")
    else:
    ...
    return {
        ...
        "sampling_shapes_correct": rx.shape == (16, 784) and ry.shape == (16,) and rt.shape == (16,),
...
def test_replay_training() -> Dict[str, Any]:
    ...
    if "loss" not in metrics or "accuracy" not in metrics:
        ...
        print(f"  PASS: train_step with replay task_id={replay_task_id} works, loss={metrics['loss']:.4f}")
...
def test_lwf_distillation() -> Dict[str, Any]:
    ...
    distill = lwf_loss.distill_only(logits1, task_id=1, prev_logits=prev_model(x, task_id=1))
    if distill is None:
        all_passed = False
...
def test_ewc_consolidation() -> Dict[str, Any]:
    ...
    if not hasattr(update, '_importance') or update._importance is None or len(update._importance) == 0:
        all_passed = False
    ...
    if not hasattr(update, '_old_params') or update._old_params is None or len(update._old_params) == 0:
        all_passed = False
    ...
    model.train()
    initial_params = {n: p.clone() for n, p in model.geometry.named_parameters() if p.requires_grad}
    ...
    return {
        ...
        "passed": all_passed,
        "fisher_computed": hasattr(update, '_importance') and update._importance is not None and len(update._importance) > 0,
        "fisher_non_zero": sum(v.abs().sum().item() for v in update._importance.values()) > 0 if hasattr(update, '_importance') and update._importance else 0,
        "opt_params_stored": hasattr(update, '_old_params') and update._old_params is not None and len(update._old_params) > 0,
...
def test_stability_guard_integration() -> Dict[str, Any]:
    ...
    if not all(hasattr(v, 'kill') and hasattr(v, 'statistic') and hasattr(v, 'threshold') for v in verdicts):
        all_passed = False
    ...
    if any(s < 0 for s in stats):
        ...
        print(f"  PASS: Statistics non-negative: min={min(stats):.4f}, max={max(stats):.4f}")
    ...
    return {
        ...
        "all_have_attributes": all(hasattr(v, 'kill') and hasattr(v, 'statistic') and hasattr(v, 'threshold') for v in verdicts),
