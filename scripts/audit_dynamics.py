...
def test_instantaneous_vs_autograd() -> Dict[str, Any]:
    ...
    for trial in range(10):
        ...
        inst_output = state.activations[-1] if isinstance(state.activations, list) else state.activations
...
def test_predictive_settling_error_decreases() -> Dict[str, Any]:
    ...
    for trial in range(n_trials):
        ...
        if overall_decrease and early_decreasing:
            n_passed += 1
            print(f"  Trial {trial}: PASS (steps={len(energy_history)}, energy: {initial_energy:.4f} -> {final_energy:.4f}, early decreasing={early_decreasing})")
        else:
            print(f"  Trial {trial}: FAIL (overall_decrease={overall_decrease}, early_decreasing={early_decreasing}, energy: {initial_energy:.4f} -> {final_energy:.4f})")
...
def scan_inplace_ops(module: nn.Module, path: str = "") -> List[Tuple[str, str]]:
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if any(op in stripped for op in ['+=', '-=', '*=', '/=', '.add_(', '.mul_(', '.sub_(', '.div_(', '.copy_(']):
                # Check if it's operating on a parameter or buffer
                issues.append((f"{path}.forward:{i+1}", f"In-place op: {stripped[:80]}"))
...
def test_inplace_op_audit() -> Dict[str, Any]:
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith('#'):
                continue
            if any(op in stripped for op in ['+=', '-=', '*=', '/=', '.add_(', '.mul_(', '.sub_(', '.div_(', '.copy_(']):
                issues.append((f"EnergyMinimizationDynamics._settle_step:{i+1}", f"In-place op: {stripped[:80]}"))
    except (OSError, TypeError):
        pass
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith('#'):
                continue
            if any(op in stripped for op in ['+=', '-=', '*=', '/=', '.add_(', '.mul_(', '.sub_(', '.div_(', '.copy_(']):
                issues.append((f"PredictiveSettlingDynamics.settle:{i+1}", f"In-place op: {stripped[:80]}"))
    except (OSError, TypeError):
        pass
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith('#'):
                continue
            if any(op in stripped for op in ['+=', '-=', '*=', '/=', '.add_(', '.mul_(', '.sub_(', '.div_(', '.copy_(']):
                issues.append((f"SpikeIntegrationDynamics.settle:{i+1}", f"In-place op: {stripped[:80]}"))
    except (OSError, TypeError):
        pass
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith('#'):
                continue
            if any(op in stripped for op in ['+=', '-=', '*=', '/=', '.add_(', '.mul_(', '.sub_(', '.div_(', '.copy_(']):
                issues.append((f"LazyStateDynamics.settle:{i+1}", f"In-place op: {stripped[:80]}"))
    except (OSError, TypeError):
        pass
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith('#'):
                continue
            if any(op in stripped for op in ['+=', '-=', '*=', '/=', '.add_(', '.mul_(', '.sub_(', '.div_(', '.copy_(']):
                issues.append((f"DiffusionDynamics.settle:{i+1}", f"In-place op: {stripped[:80]}"))
    except (OSError, TypeError):
        pass
    
    # Also check FeedforwardGeometry.route
    ff_geometry = FeedforwardGeometry(GeometryConfig.feedforward(input_dim=10, output_dim=5, hidden_dims=(20,)))
    ...
    try:
        ...
        loss = free_state.activations[-1].sum() if isinstance(free_state.activations, list) else free_state.activations.sum()
...
def test_device_consistency() -> Dict[str, Any]:
    ...
    def run_on_devices(geometry_factory, dynamics_factory, substrate_factory, 
                       x_factory, y_factory, get_output):
    ...
    def run_inst_settle(geom, sub, dyn, x, y):
        state = SystemState(x=x)
        state = dyn.settle(state, geom, sub, target=None)
        return state.activations[-1] if isinstance(state.activations, list) else state.activations
