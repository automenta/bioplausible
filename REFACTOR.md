...
**Reduction so far**: ~19,050 lines (46%) via structural consolidation (Sprint 2.3 is a substrate re-platforming, not a deletion). **Target run-rate remaining**: the `equitile/` subsumption teardown (see below) — the planned deployable consolidations are done.
...
|----------|------|-------|----------|--------|--------|
| 1 | **Legacy Core Teardown (Sprint 2.2)** | ~12,000 | — | 🟢 **NEXT** | Large |
| 2 | `equitile/benchmarks/` migration to substrate | ~1,500 | #1 | 🟡 | Medium |
| 3 | `equitile/utils/reproducibility.py` → `bioplausible.core.utils` | ~200 | — | 🟡 | Small |

**Total remaining in `equitile/`**: ~13,700 lines (legacy core + benchmarks + utils). The substrate-bound deployments (vision/graph/rl/timeseries), `TileLM`, `validate.py`, and `utils/reproducibility.py` are the only pieces that survive the subsumption.
