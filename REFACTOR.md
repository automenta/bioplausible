...
**Reduction so far**: ~19,050 lines (46%). **Target run-rate remaining**: next realizable (Graph/Temporal/RL deployment model classes → substrate model classes, remaining `equitile/` utils cleanup).
...
| 4 | **Graph/Temporal/RL deployment model classes → substrate model classes** | ~1,400 | #1 (need substrate growth for RL dynamics?) | 🟢 **NEXT** | Medium |
...
**Total remaining in `equitile/`**: ~1,400 lines (deployment model classes #4 — the whole `equitile/` tree was 12,442 lines; remaining substrate-bound deployments + `validate.py` + `benchmarks/` + `utils/reproducibility.py` + `TileLM` re-export).
...
### Sprint 2.3 — Deployment Model Classes (after #2)

Consolidate `GraphEquiTile`, `TimeSeriesEquiTile`, `RLEquiTile` onto substrate model-class platform (like `ConvEquiTile` in Sprint 1.0). Feature-extraction wiring already done (Sprint 1.1); top-level model classes need substrate-native `train_step`/`forward` implementations. `ConvEquiTile` already substrate-native (Sprint 1.0).
