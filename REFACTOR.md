...
**Completed**: ~3,800 lines saved (9.3%) across 90+ files; **§1/§2/§3(target 1)/§4(metrics rename)/§7/§8(EquiTile Generification)/§9(Training Loop) complete**; §3 Config Unification targets 2-4 standardized on BaseConfig; §4 type-clean, §10 data/mnist + toy dedup landed; **Checkpoint/serialization unified on `core.checkpoint`** (CoreTrainer + LMTrainer + FastLMEquiTile); **Training-loop PoC + 3 rollouts** (`supervised_step` → `eqprop/_unified.py`, `core/ebm.py`, `forward_only.py`, `predictive_coding.py`); **EquiTile Generification (§8)** — generic `TileAlgorithm` with 5 static factories + 3 dynamics protocols in `core/local_learning/algorithm.py` + `TileFA` validation in `zoo/models/tile_fa.py`; `MultiOptimizerMixin` groups wired; bio-plausible loop (`local_update`) + autograd baseline (`train_step`).
...
**Total verified reduction**: ~3,800 lines (9.3%)
...
| Training Loop Infra | ~20 implementations | 4 | 16 | 🟡 **Rollout in progress** (PoC + 3 rollouts done) |
...
**Pattern proven**: `supervised_step` canonicalizes plain-BPTT; 3 rollouts done.

**Rollout candidates** (train_step bodies that are exactly CE+optimizer, no custom physics):
1. `zoo/models/predictive_coding.py:263` (`PredictiveCodingHybrid`) — **NEEDS EXTENSION**: returns `cls_loss` + `pc_loss`; `supervised_step` returns only `loss/accuracy`. Option: extend `supervised_step` with optional `extra_keys` return, or keep bespoke.
2. Remaining plain-BPTT train_steps in `zoo/models/` (audit required — ~10 candidates).

**Migration steps per model**:
1. Replace manual `zero_grad/forward/CE/backward/clip/step` with `supervised_step(model, optimizer, x, y, grad_clip=...)`
2. If model returns extra keys (e.g., `cls_loss`), extend `supervised_step` with `return_extra=True` or keep tail call.
3. Verify: `ruff check --fix . && pyright . && pytest --cov`
...
| Training Loop | ~200 lines (4 of 20 done) 🟡 |
