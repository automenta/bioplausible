...
| 3 | `equitile/core/model.py` (legacy EquiTile, 1,305 lines) | 1,305 | — (analysis no longer depends on it) | 🟢 **NEXT** | Large |
...
**Total remaining in `equitile/`**: ~6,600 lines across the 6 itemized packages above (the whole `equitile/` tree is 12,442 lines incl. `__init__.py` + `validate.py` + deployments graphs/vision/rl/timeseries, which are substrate-bound per Sprint 1.0/1.1 but not yet consolidated as model classes — tracked under #4).

### ✅ Sprint 2.1 — `equitile/analysis/` Substrate Port (done)
