...
|----------|------|-------|----------|--------|--------|
| 2 | `equitile/benchmarks/` relocate → `bioplausible/benchmarks` | ~1,500 | — | 🟡 **NEXT** | Medium |
| 3 | `equitile/utils/reproducibility.py` → `bioplausible.core.utils` | ~200 | — | 🟡 | Small |

**Total remaining in `equitile/`**: ~1,700 lines (benchmarks + utils). The substrate-bound deployments (vision/graph/rl/timeseries), `TileLM`, `validate.py`, and `utils/reproducibility.py` are the only pieces that survive the subsumption.

> **Next-session guidance**: the `equitile/` package is now a *thin* deployable surface — no legacy model code, no legacy config, no legacy access anywhere (the `equitile` registry name and `EquiTile` are gone; uses are `tile_pc`/`tile_*` substrate models). The two remaining items are pure relocations:
> 1. **`equitile/benchmarks/`** (`rigorous.py` + `compare_nanoGPT.py` + `efficiency_analysis.py`) is substrate-ready — it already builds `TileLM` directly and its `"equitile"` strings are just result-labels/`model_type`, **not** registry lookups. Relocate to `bioplausible/benchmarks/` and repoint `equitile/benchmarks/__init__.py` re-exports + `test_lm_demo.py` bench imports. `test_lm_demo.py` imports `bioplausible.equitile.benchmarks.compare_nanoGPT` + `efficiency_analysis` — update those import paths.
> 2. **`equitile/utils/reproducibility.py`** → move to `bioplausible.core.utils.reproducibility`; repoint `equitile/validate.py` (`from bioplausible.equitile.utils import ReproducibilityTracker, set_reproducible_mode`) and `test_reproducibility.py`. Then delete the `equitile/utils/` package.
> 3. After both, `equitile/utils/` and `equitile/benchmarks/` can be deleted and `equitile/` shrinks to deployments + `validate.py` + `__init__.py`. Note the `equitile` *package path* must stay (deployment models + their re-exports live there and are imported by surviving tests), even though the `equitile` *model name* is gone.
