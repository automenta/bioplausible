...
**Completed (this session #10 — TP bespoke consumption: the second inverse-network family driven through the dispatch seam):**
- **`TPKernelBackend.kernel_train_step`** (`acceleration/tp_kernels.py`): the Difference-Target-Propagation backend now exposes the bespoke step, mirroring the reference `DifferenceTargetProp.train_step` **exactly**: forward collecting activations → output-layer CE via its own Adam (`out_opt`) → difference target `h − target_lr·∇ₕL` built from the output layer under `torch.enable_grad` → backward target propagation through the per-layer `inverse_net` Sequentials (`t_prev = h_prev − inv(h_curr) + inv(t_curr)`) → per-layer forward-net fitting-to-target + inverse cycle-consistency, each with its own per-layer Adam (`opt_f`/`opt_g`), in the reference's exact order (inverse fits before `opt_f.step()`). Binds `_forward_layers`/`_inverse_layers` from the model's `DTPLayer.forward_net[0]`/`inverse_net[0]` so memory stats stay live. Returns `None` (caller falls through) when the model lacks the `layers`/`out_layer` surface. Verified: after 6 synthetic passes ref=0.3625 vs kernel=0.3625 (**delta=0.0000**). The bespoke route is authoritative for TP — it gates the uniform consumer out so DTP's per-layer Adams are never mis-stepped.
- **Dispatch tests** (`test_kernel_dispatch.py`, now 13): `test_tp_kernel_train_step_consumed` — `diff_target_prop` + `use_kernel=True` attaches `TPKernelBackend`, drives one `_train_step` (finite loss, layer-0 `forward_net` weights changed, `"kernel"` path counter ≥1); `test_kernel_tp_learns` — kernel-driven DTP reaches ≥0.2 on the separable task and stays within 1% of the reference path (asserts `abs(kernel−ref) ≤ 0.01`).
- **Accuracy parity** (`test_kernel_accuracy_parity.py`, now 8): added `("diff_target_prop", "tp", {input_dim/hidden_dim/output_dim/num_layers, learning_rate=1e-3, target_lr=0.1})`; `test_kernel_accuracy_parity_digits[diff_target_prop]` and the synthetic variant both PASS. **Bespoke-dynamics accuracy parity now proven for FA, Backprop, PEPITA, and TP.**
...
## Session Progress (this working session #13 — Final Validation & Status Update):

**Completed:**
- **Full kernel test suite validation**: All kernel backend tests passing (97 passed, 2 skipped, 1 xfailed, 1 xpassed across `test_family_kernel_parity`, `test_kernel_parity_base`, `test_backprop_parity`, `test_kernel_dispatch`).
- **Integration verification**: `check_imports` and `check_seams` both green — no layer violations, no import cycles, all seam gates hold.
- **Export pipeline functional**: `biopl-export-kernel` CLI works end-to-end, writing hardware manifest + state dict + best-effort ONNX for all registered backends.
- **Benchmark harness operational**: `tools/benchmark_all_kernels.py` sweeps 11 families × 34 pair-entries (CPU/CUDA/TRITON/neuromorphic), emits `artifacts/kernel_benchmark_report.json` with status/wall-time/peak-memory/telemetry.
- **Documentation consolidated**: `docs/kernel_backend_guide.md` and `docs/hardware_targets.md` copied to `docs/api/` with `docs/api/index.md` linking them.
- **Contrastive kernels registered in KernelRegistry**: Verified FA and Hebbian contrastive kernels appear in `KernelRegistry.list_for()` for CPU/CUDA/TRITON targets.

**Known Issues:**
- `test_contrastive_kernel_learns` fails (acc=0.100, chance=0.1) due to `standard_fa` model mismatch — the model uses spectral normalization and an Adam optimizer, while the contrastive FA kernel expects plain `nn.Linear` layers. The dispatch seam works correctly (`test_contrastive_kernel_attached_and_consumed` passes); the fix is to use a plain Linear-stack model for the contrastive learning test.
- Mixed precision (FP16/BF16/INT8) validation not yet implemented — `supported_dtypes` is tagged per backend but parity tests only run at FP32.
- Full-suite regression test not explicitly run — though all targeted kernel tests pass.
...
**Next phases to pick up:** **Mixed precision validation (FP16/BF16/INT8)** across all kernel backends — `supported_dtypes` already tagged per backend; need parity tests at each precision. **Full-suite regression test** (all kernel + integration + export tests in one run). **Contrastive kernel learning test fix**: replace `standard_fa` with a plain `nn.Linear` stack model in `test_contrastive_kernel_learns` to prove the O(1) contrastive path learns. **Phase 2-9 accuracy parity** on digits is proven for FA, Backprop, PEPITA, and TP. Remaining families for the accuracy-parity suite: Hebbian (blocked — `status_tag("broken")` models), PC (reference is plain `supervised_step` BPTT, no bespoke dynamics to mirror), SNN (reference doesn't learn on the probe — acc stuck at chance), Tile, MEP, O1Memory (optimizer-scheduler semantics). **Phase 2 MEP Triton kernels** implemented (`MEP_TritonOps.fisher_whiten`, `ep_settle` full Triton; `muon_orthogonalize`, `dion_update` PyTorch fallback). **Phase 3 SettleProtocol** unified (`SettleProtocol`, `SettleConfig`, `SettleTelemetry`, `settle_universal`). **Hebbian still will not bind through ... (line truncated to 2000 chars)
---
...
- [x] `TPKernelBackend`
- [x] Parity (finite/shape): covered by `test_family_kernel_parity.py`
- [x] **Bespoke consumption (session #10)**: `TPKernelBackend.kernel_train_step` mirrors the reference `DifferenceTargetProp.train_step` exactly (per-layer Adams, target propagation through inverse nets). Driven for `diff_target_prop` through the dispatch seam; dispatch tests `test_tp_kernel_train_step_consumed`/`test_kernel_tp_learns` + accuracy parity digits/synthetic PASS (kernel == reference, delta=0.0000).
