...
- The generic consumer currently handles **uniform-interface backends only** (FA, Backprop, Hebbian have `forward(x)->(out, acts)`, `backward(acts, error)->grads`, `update_weights(grads, lr)`). Families with bespoke dynamics (FF/PEPITA two-pass, TP inverse nets, PC inference, SNN simulate, Tile/MEP settle) still need their own consumption wiring — a future `dispatch_train_step` case per family, or a `kernel_train_step` method on the backend.
...
**Next phases to pick up:** Phase 2-9 end-to-end **accuracy** parity (MNIST/CIFAR) for each family — **FA learning parity is now proven** (`test_kernel_backend_matches_reference_learning`, delta=0.000 vs reference on a synthetic separable task), and **Backprop is now consumed and learns through the dispatch seam** (`test_backprop_kernel_consumed_in_train_step` + `test_backprop_kernel_learns`). The other families (Hebbian, FF/PEPITA, TP, PC, SNN, Tile, MEP, O1Memory) still lack a learning-parity suite. Now that `_resolve_kernel_layers` generalizes the uniform consumer to `.net`/`transition_modules()`, **Hebbian should bind if its model exposes a plain Linear stack** (its registered models use `W_in`/`chain`/`head` and spectral-norm-wrapped/Conv layers, so they still won't conform — a future `kernel_train_step` backend method or per-family `dispatch_train_step` case is the route). The bespoke-dynamics families (FF/PEPITA, TP, PC, SNN, Tile, MEP, O1Memory) still need their own consumption wiring. Also open: give `backprop_mlp`-style uniform models a model-side `optimizer` (or let the consumer fall back to the trainer's optimizer/LR) so the raw-SGD `update_weights` fallback trains at the reference LR instead of `config.learning_rate`; Phase 11 export pipeline is now scaffolded (`acceleration/export.py` + `biopl-export-kernel`), with the real HLS/Verilog/NxSDK/SPICE generators + trained-weight CLI binding + docs still open; Phase 12 `SettleProtocol`/`settle_universal` consolidation + `TrainingMetrics.extra["settle_telemetry"]` (the `_run_kernel_train_step` seam now surfaces `get_settle_telemetry()` into `metrics["extra"]["settle_telemetry"]`, but it only fires when a *settling* backend is driven through the seam — settling backends are bespoke and still not consumed) + canonical-hash content fix + dead-code sweep (the 7.4 greps are already clean; the 7.3 canonicalizer already handles tensors via `.tolist()`).
