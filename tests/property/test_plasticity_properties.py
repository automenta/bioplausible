...
def _build_registry(geometry: RecurrentGeometry, config: _TestContextConfig):
    ...
    if config.plasticity_type == "routing":
        ...
        dummy_plastic["fast_weights"] = torch.zeros(config.batch_size, config.fast_weight_dim)
        ...
        dummy_plastic["operator_logits"] = torch.zeros(config.batch_size, config.num_operators)
...
def _create_test_context(config: _TestContextConfig = None):
    ...
    dummy_substrate: dict[str, Tensor] = {"conductance": torch.randn(config.batch_size, 20)}
    registry.validate(
        CompositeState(
...
@given(
    ...
    temp=st.floats(min_value=0.1, max_value=10.0, allow_nan=False, allow_infinity=False),
    seed=st.integers(min_value=0, max_value=2**32 - 1),
)
@settings(max_examples=50, deadline=None)
def test_routing_plasticity_initial_psi_shapes(gate_dim, batch_size, decay, lr, temp, seed):
    """initial_psi returns correctly shaped tensors for any valid config."""
    torch.manual_seed(seed)
    plasticity = RoutingPlasticity(gate_dim=gate_dim, decay=decay, learning_rate=lr, temperature=temp)
...
@settings(max_examples=50, deadline=None)
def test_routing_plasticity_no_nan_inf_gate_logits(gate_dim, batch_size, decay, lr, temp, seed):
...
@settings(max_examples=50, deadline=None)
def test_routing_plasticity_gate_logits_bounded(gate_dim, batch_size, decay, lr, temp, seed):
...
def test_routing_plasticity_idempotence_same_gates(gate_dim, batch_size, decay, lr, temp, seed):
    ...
    # Test with top_k
    plasticity_topk = RoutingPlasticity(gate_dim=gate_dim, decay=decay, learning_rate=lr, temperature=temp, top_k=min(2, gate_dim))
...
@given(
    ...
    batch_size=st.integers(min_value=1, max_value=4),
    decay=st.floats(min_value=0.0, max_value=0.99, allow_nan=False, allow_infinity=False),
    lr=st.floats(min_value=0.0, max_value=0.5, allow_nan=False, allow_infinity=False),
    seed=st.integers(min_value=0, max_value=2**32 - 1),
)
@settings(max_examples=30, deadline=None)
def test_routing_plasticity_decay_bounds(gate_dim, batch_size, decay, lr, seed):
    ...
    torch.manual_seed(seed)
    plasticity = RoutingPlasticity(gate_dim=gate_dim, decay=decay, learning_rate=lr, temperature=1.0)
...
@given(
    ...
    decay=st.floats(min_value=0.0, max_value=1.0, allow_nan=False, allow_infinity=False),
    lr=st.floats(min_value=0.0, max_value=1.0, allow_nan=False, allow_infinity=False),
    scale=st.floats(min_value=0.0, max_value=10.0, allow_nan=False, allow_infinity=False),
    seed=st.integers(min_value=0, max_value=2**32 - 1),
)
@settings(max_examples=50, deadline=None)
...
def test_fast_weight_plasticity_no_nan_inf(
    ...
    context, geometry, _ = _create_test_context(
        _TestContextConfig("fast_weights", fast_weight_dim=fast_weight_dim, batch_size=batch_size)
    )
    z = _make_composite_state(
        geometry, psi, {"conductance": torch.randn(batch_size, 20)}, batch_size
    )
...
@given(
    ...
    batch_size=st.integers(min_value=1, max_value=4),
    decay=st.floats(min_value=0.5, max_value=0.99, allow_nan=False, allow_infinity=False),
...
def test_null_plasticity_step_identity(batch_size, seed):
    ...
    context, geometry, _ = _create_test_context(_TestContextConfig("null", batch_size=batch_size))
    z = _make_composite_state(
        geometry, {"anything": torch.randn(batch_size, 10)}, {"conductance": torch.randn(batch_size, 20)}, batch_size
    )
    z.activity["x"] = torch.randn(batch_size, 10)
...
def test_rule_state_plasticity_initial_psi_shapes(
    ...
    assert torch.allclose(psi["operator_logits"], torch.zeros_like(psi["operator_logits"]))
    assert torch.allclose(psi["controller_state"], torch.zeros_like(psi["controller_state"]))
...
def test_rule_state_plasticity_no_nan_inf(num_operators, batch_size, decay, lr, seed):
    ...
    context, geometry, _ = _create_test_context(
        _TestContextConfig("rule_state", num_operators=num_operators, batch_size=batch_size)
    )
    z = _make_composite_state(
        geometry, psi, {"conductance": torch.randn(batch_size, 20)}, batch_size
    )
...
def test_rule_state_plasticity_get_active_operator(num_operators, batch_size, seed):
    ...
    assert torch.allclose((weights_eval == 1.0).sum(dim=-1).float(), torch.ones(batch_size, dtype=torch.float))
...
def test_create_fast_weight_plasticity_factory(fast_weight_dim, decay, lr, scale):
    ...
    config = PlasticityConfig.fast_weights(
        fast_weight_dim=fast_weight_dim, decay=decay, learning_rate=lr, outer_product_scale=scale
    )
...
@settings(max_examples=20, deadline=None)
def test_fast_weight_plasticity_integration_with_joint_context(fast_weight_dim, batch_size, seed):
