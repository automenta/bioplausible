...
def train_system(system, train_loader, val_loader, epochs: int, device: str, seed: int = 42):
...
def train_zoo_model(model, train_loader, val_loader, epochs: int, device: str, seed: int = 42):
...
class TestBackpropParity:
    ...
    def test_create_backprop_mlp_matches_native(self, epochs):
        ...
        system1 = create_backprop_mlp(input_dim, (hidden_dim,), output_dim, lr=0.001, device=device)
        system2 = create_native_backprop_mlp(input_dim, hidden_dim, output_dim, lr=0.001)
...
class TestEqPropParity:
    ...
    def test_create_eqprop_mlp_matches_native(self, epochs):
        ...
        system1 = create_eqprop_mlp(input_dim, (hidden_dim,), output_dim, beta=0.1, n_iters=10, lr=0.01, device=device)
        system2 = create_native_eqprop_mlp(input_dim, hidden_dim, output_dim, beta=0.1, n_iters=10, lr=0.01)
...
class TestFAParity:
    ...
    def test_create_fa_mlp_matches_native(self, epochs):
        ...
        system1 = create_fa_mlp(input_dim, (hidden_dim,), output_dim, lr=0.001, feedback_scale=0.1, device=device)
...
class TestForwardForwardParity:
    ...
    def test_create_ff_mlp_matches_zoo(self, epochs):
        ...
        system1 = create_ff_mlp(input_dim, (hidden_dim, hidden_dim), output_dim, 
                                layer_lr=0.03, classifier_lr=0.01, threshold=2.0, 
                                num_layers=2, device=device)
        model2 = ForwardForwardNet(input_dim, hidden_dim, output_dim, 
                                   num_layers=2, layer_lr=0.03, classifier_lr=0.01).to(device)
...
class TestPEPITAParity:
    ...
    @pytest.mark.skipif(not PEPITA_NATIVE_AVAILABLE, reason="Native PEPITA not available")
    @pytest.mark.parametrize("epochs", [3])
    def test_native_pepita_matches_zoo(self, epochs):
        ...
        system1 = create_native_pepita_mlp(input_dim, hidden_dim, output_dim, num_layers=2, lr=0.01)
        model2 = PEPITA(input_dim, hidden_dim, output_dim, num_layers=2, lr=0.01).to(device)
...
class TestOntologyComposition:
    ...
    @pytest.mark.parametrize("credit_type,expected_accuracy", [
        ("gradient", 85.0),
    ...
    def test_credit_assignment_composition(self, credit_type, expected_accuracy):
        ...
        geometry = FeedforwardGeometry(GeometryConfig.feedforward(input_dim, output_dim, (hidden_dim,), init_scale=0.1))
        dynamics = InstantaneousDynamics(StateDynamicsConfig.instantaneous())
        
        credit_map = {
            "gradient": lambda: ThermodynamicContrast(CreditAssignmentConfig(credit_type="gradient", beta=0.5, feedback_matrix=None, local_objective="mse", orthogonal_init=False, feedback_scale=0.01)),
            "thermodynamic_contrast": lambda: ThermodynamicContrast(CreditAssignmentConfig.thermodynamic_contrast(beta=0.1)),
            "random_projections": lambda: RandomProjectionsCredit(CreditAssignmentConfig.random_projections(feedback_scale=0.1)),
            "local_goodness": lambda: LocalGoodnessCredit(CreditAssignmentConfig.local_goodness()),
        }
        ...
        assert acc > expected_accuracy, f"{credit_type}: {acc:.1f}% < {expected_accuracy}%"


class TestSubstrateVariants:
    """Test that all substrate types compose correctly."""
    
    @pytest.mark.parametrize("substrate_type", [
    ...
    def test_substrate_composition(self, substrate_type):
        ...
        from bioplausible.core.ontology import (
            AnalogSubstrate, MemristiveSubstrate, NeuromorphicSubstrate,
            OpticalSubstrate, QuantumSubstrate, BackpropCredit,
        )
        ...
        substrate_map = {
            ...
            "memristive": lambda: MemristiveSubstrate(SubstrateConfig.memristive(device=device)),
            "neuromorphic": lambda: NeuromorphicSubstrate(SubstrateConfig.neuromorphic(device=device)),
        ...
        geometry = FeedforwardGeometry(GeometryConfig.feedforward(input_dim, output_dim, (hidden_dim,), init_scale=0.1))
        dynamics = InstantaneousDynamics(StateDynamicsConfig.instantaneous())
        credit = BackpropCredit(CreditAssignmentConfig(credit_type="gradient", beta=0.5, feedback_matrix=None, local_objective="mse", orthogonal_init=False, feedback_scale=0.01))
