...
class TestRuleStatePlasticity:
    ...
    def test_step_updates_operator_logits(self, device):
        ...
        assert not torch.allclose(psi_after["controller_state"], psi["controller_state"])
...
class TestPlasticityDeviceConsistency:
    ...
    @pytest.mark.parametrize("plasticity_factory", [
        lambda d: FastWeightPlasticity(fast_weight_dim=512, decay=0.9, learning_rate=0.1).to(d),
        lambda d: RoutingPlasticity(gate_dim=64, decay=0.99, learning_rate=0.01),
        lambda d: RuleStatePlasticity(num_operators=8, operator_dim=64, device=d),
        lambda d: NullPlasticity(),
    ])
    def test_plasticity_device_consistency(self, plasticity_factory, device):
        ...
        elif isinstance(plasticity, NullPlasticity):
            pass  # Nothing to check
        
        # RoutingPlasticity has no internal persistent tensors
        elif isinstance(plasticity, RoutingPlasticity):
            pass  # Nothing to check
