...
class TestDevicePropagation:
    ...
    @pytest.mark.parametrize("plasticity_type", ["null", "fast_weights", "routing", "rule_state"])
    def test_joint_system_to_device(self, plasticity_type, device_cpu, device_cuda):
        ...
        if plasticity_type == "fast_weights":
            ...
            assert continual.plasticity._operator_embeddings.device.type == device_cuda.type
    ...
    def test_dynamics_to_device(self, device_cpu, device_cuda):
        ...
        dynamics2 = EnergyMinimizationDynamics(StateDynamicsConfig.energy_minimization(max_steps=10))
...
class TestComponentDeviceConsistency:
    ...
    def test_rule_state_plasticity_device(self, device_cpu, device_cuda):
        ...
        plasticity = RuleStatePlasticity(num_operators=8, operator_dim=64, device=device_cuda)
...
class TestCPUCUDAConsistency:
    ...
    def test_fast_weight_plasticity_deterministic_per_device(self, device_cpu):
        ...
        # Create on CPU
        plasticity_cpu = FastWeightPlasticity(fast_weight_dim=512, decay=0.9, learning_rate=0.1)
        ...
        # Create on CUDA
        plasticity_cuda = FastWeightPlasticity(fast_weight_dim=512, decay=0.9, learning_rate=0.1)
