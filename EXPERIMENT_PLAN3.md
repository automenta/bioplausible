...
- **Success:** `validate_all_rule_spaces()` exits 0; the `neural_cube` effective search space equals its constructor surface. A future contributor adding a phantom knob gets a CI failure, not a quiet probe waste.

This is the single highest-leverage engineering step in the project: it is cheap, it retroactively tells you which prior numbers are trustworthy, and it *prevents the §12 bug from ever recurring on any future family*. The framework protects the next 50 algorithms, not just this one.
...
### The design — `EquilibriumSettleMixin`

- **Action:** introduce `zoo/_settling.py::EquilibriumSettleMixin` (or a thin `settle_state(self, x_proj, h0, steps) → (h, steps_taken, converged)` helper the mixin calls) that:
  1. exposes `convergence_threshold: float = 1e-3` and `convergence_start: int = 5` as constructor args (forwarded to the mixin, absorbed by siblings with `**kwargs`),
  2. routes the equilibrium loop through `settle_activations_list` (the one early-stop path),
  3. records `steps_taken`/`converged` so the probe driver surfaces "did early-stop fire" as a metric.
- **`NeuralCube`** adopts the mixin and replaces its `for _ in range(steps)` (`neural_cube.py:175`) with `self.settle_state(...)`. **Now the §7 win applies to `neural_cube` with zero per-algorithm code** — the same mixin lifts `DeepEP`/`SparseEquilibrium`/`LazyEqProp`/`HolomorphicEP` as they adopt it.
- **Searchability:** `convergence_threshold ∈ [1e-4, 1e-2]` (log) and `convergence_start ∈ [2, 10]` (int) become searchable for *any* mixin-bearing rule — added to `RULE_SPACES["neural_cube"]` in the same PR that lands the model change (P0a discipline: space and constructor move together, never one ahead of the other).
...
**Success:** `NeuralCube` settles via the shared primitive; `convergence_threshold`/`convergence_start` are real constructor args and searchable; a unit test asserts every mixin-bearing model's `forward` *actually* early-stops (a `convergence_threshold=1.0` model terminates in `< max_steps` with `converged=True`). The §7 win is no longer an `eqprop`-only result.
...
- **Reality:** `NeuralCube`'s activation memory scales as `cube_size**3` per step (`n_neurons = cube_size**3`). The 6.4× memory is structural to the architecture, not a training-loop overhead. Optimization candidates (each a **controlled experiment**, PLAN2 §7 discipline — implement/measure/compare/keep-or-revert):
  ...
  - **Activation checkpointing on the settle loop** (PLAN2 §2): ~2× compute, ~O(√steps) memory — directly attacks the per-step activation footprint. This is a *framework* optimization (applies to any mixin-bearing rule), not a `neural_cube` patch.
...
- **P5b — Hardware × equilibrium cross-terms (PLAN2 §18 "not-yet"):** on a substrate (`fpga`), does `convergence_threshold` search behave the same, or does quantization change where early-stop pays? Record the interaction as a searchable config (via the P1 mixin — the cross-term is *free* to expose once the mixin lands). This is the actual product differentiation: substrate-specific optimization, not a GPU-parity number.
...
⊕ *If `neural_cube` is de-prioritized tomorrow, P0/P1/P2/P3a still compound: the integrity gate protects the next algorithm, the mixin carries the §7 win to every equilibrium rule, the read-half serves any conditional in the KB, and the memory levers (checkpointing especially) are framework-level. That invariance — not the success of any one rule — is what "the framework is the product" means in code.*
