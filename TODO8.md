...
| 3.3 | PEPITA `LocalGoodnessCredit`: empty pseudo-gradients | **Root cause found (R5b-0 audit, 2026-08-31): the method is an unimplemented stub** — `compute_pseudo_gradient` returns `[]` (marked "Simplified"), as do TemporalTrace/TargetInversion/Homeostatic. Fix = implement the bodies, not debug wiring (improvement-21) |
...
→ R5b-A pre-register (theory-driven, broken-instrument kill criterion)
...
Then R5b proceeds fidelity-gated (see R5b Ordering): **next = R5b-A pre-registration** on the 8-coordinate validated subspace or a theory-driven hypothesis whose both arms pass the gate; instrument repairs (improvements 20-22: metric leakage, credit stubs, plasticity wiring) widen the manifest before any effect claim. Zoo deletion and kernel breadth stay non-blocking.
...
20. **Reported accuracy carries supervision leakage for settle-based dynamics (2026-08-31, R5b-0 audit)** — `run_train_step` computes metrics on the NUDGED settled state, whose output layer received `beta * (target - output)` during settling; `evaluate_episode` records that as `task_accuracy`. A coordinate whose credit is a stub (local_goodness, Δθ = 0.00e+00) "reaches" 0.96 accuracy while its target-free settle accuracy is chance (0.13). Learning claims require post-update target-free forward accuracy (or an explicit "nudged-settle fit" label). Fix shape: add a `free_forward` metric pass after `update.step` in the episode protocol, or record both metrics with distinct names. This is the single largest instrument defect found by the gate.
21. **Credit stubs: `LocalGoodnessCredit`, `TemporalTraceCredit`, `TargetInversionCredit`, `HomeostaticCredit` `compute_pseudo_gradient` bodies are `return []` (2026-08-31)** — marked "Simplified" in `ontology/credit.py`. R3.3's "PEPITA empty pseudo-gradients — debug credit path" is thereby root-caused: there is no path to debug; the method is unimplemented. R3.3's fix shape becomes "implement the pseudo-gradient bodies" (PEPITA per-layer goodness, STDP correlations, target inversion, homeostatic scaling). Also note `RandomProjectionsCredit` under energy_minimization produces a ~10⁻⁴-scale signal (Δθ ≈ 5.7e-06 per episode) — non-empty per spec but near-inert; validate its usability before RP-arm claims.
22. **Plasticity is not wired into the episode pipeline (2026-08-31, R5b-0 audit)** — `run_train_step(substrate, geometry, dynamics, credit, update, x, y)` takes no M-axis: `plasticity.step` is never invoked and `initial_psi`'s ψ is discarded. ψ updates currently exist only in `core/continual/training.py` (continual path) and the `CoupledTransition` machinery — not in `JointSystem.train_step`, so every campaign episode runs M=null regardless of the coordinate's plasticity value. Fix shape: thread ψ through `run_train_step` (settle → `plasticity.step(psi, z, context)` → ψ-modulated forward/credit per the M-primitive contract), keeping the J2/J3 invariants; the fidelity gate's M-verdict (`tests/property/test_campaign_fidelity.py::test_nonnull_plasticity_is_inert_in_episode_pipeline`) is the lock that flips when this lands.
