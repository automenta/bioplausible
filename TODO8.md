...
> 10 seed×family strata — observed, NOT interpreted (no pre-registration yet)** ·
> gate `pytest -q` green (1260 passed), pyright ontology 0 errors, integration 218 passed ·
> **Next (Execution Order 10): R5b-A pre-registration on the 48-coordinate manifest.**
>
...
| A | Pre-register one toy-scale hypothesis | From RESEARCH3 L1/L2/substrate-ablation catalog. Hypothesis is selected on **theoretical grounds from the RESEARCH3 catalog, or by grid coverage — never by observed delta magnitude in R5.1c**, because unvalidated implementations can produce arbitrarily large or small spurious deltas. The pre-registration must include: (a) the implementation-fidelity preconditions for *both* arms, (b) a **"broken instrument" kill criterion** distinct from a **"null effect"** outcome, and (c) an explicit statement that a fidelity failure renders the trial inconclusive, not a disconfirmation |
| B | Locked grid campaign on GPU | `CampaignStack.run_campaign` over ~30–100 coordinates **restricted to fidelity-passing coordinates** (feed the filtered grid to `grid_sampler`), matched budgets, ≥5 seeds; replication gate (≥2 task families) must pass |
...
→ R5b-A pre-register (theory-driven, broken-instrument kill criterion; 48-coordinate manifest)   (= Execution Order 10, next — note the stable ±0.8 dynamics delta must be pre-registered or explicitly killed, never interpreted unregistered)
→ R5b-B locked campaign (restricted to fidelity-passing coordinates)
→ R5b-C defect-filtered evidence chain
...
10. **R5b-A pre-registration** on the widened 48-coordinate manifest (theory-driven, broken-instrument kill criterion). The 8-coordinate R5.1c subspace is superseded; if a hypothesis needs axes still outside the manifest, its repair (not the delta) is the deliverable. The post-repair ±0.8 dynamics delta (EM vs instantaneous over FA-live coordinates, stable across all 10 strata) is the obvious candidate — pre-register its threshold and the broken-instrument criterion, or explicitly declare it unclaimed

Then R5b-B/C/D as specified (R5b Ordering). Zoo deletion (R2.1) and kernel breadth (R4) stay non-blocking.
