...
> **imp-20 ✅, imp-21 ✅, R3.1 ✅, imp-22 ✅ — fidelity manifest widened 8/72 → 30/72
> (300/720 R5.1c records survive the filter); gate `pytest -q` green (1244 passed), pyright
> ontology 0 errors. Next (Execution Order 9 → 10): R3.2/R3.9 reclassification of
> instantaneous×contrastive + dynamics-probe coverage (predictive_settling/spike_integration/
> diffusion are "blocked"-verdict, no probe yet) → R5b-A pre-registration on the 30-coordinate manifest.**
>
...
verdicts (see `r51c/records/fidelity_report.md`): local_goodness credit = unimplemented stub
(R3.3); plasticity not wired into the episode pipeline (R3.6-adjacent, new improvement-22);
contrastive credits structurally zero under InstantaneousDynamics (R3.2); reported-accuracy
supervision leakage (new improvement-20).

| # | Blocker | Fix shape |
|---|---------|-----------|
| 3.1 | ✅ **Fixed 2026-08-31** — `DiffusionDynamics.settle` seeds fresh `requires_grad_` leaves per Langevin step and runs its internal `autograd.grad` under `torch.enable_grad()`; diffusion integration tests un-xfailed (4/4). Remaining: dynamics probes for diffusion/predictive_settling/spike_integration so those coordinates get real verdicts instead of "blocked" (Execution Order 9) |
| 3.2 | FA + `InstantaneousDynamics`: no error signal (free=nudged) | **Root cause confirmed by R5b-0 gate (2026-08-31): structural, not a bug in FA** — `InstantaneousDynamics.settle` ignores `target`, so free ≡ nudged and every contrastive credit (thermodynamic_contrast, random_projections) is identically zero (pinned by `test_contrastive_credits_dead_under_instantaneous`). Fix shape: declare the pairing invalid (R3.9 `IncompatibleCoordinateError`) and/or give contrastive credits an autograd-capable instantaneous path |
| 3.3 | ✅ **Fixed 2026-08-31 (imp-21)** — all four credit bodies implemented (LocalGoodness/TemporalTrace/TargetInversion/Homeostatic); fidelity credit probe passes for all 7 credit values on their valid pairings. Residual: recurrent×local_goodness under EM fails (settle graph not preserved) and instantaneous×contrastive stays structurally zero (R3.2, Execution Order 9) |
...
→ R3.2/R3.9 reclassification + dynamics-probe coverage                     (= Execution Order 9, next)
→ R5b-A pre-register (theory-driven, broken-instrument kill criterion; 30-coordinate manifest)   (= Execution Order 10)
...
9. **R3.2/R3.9 reclassification + probe coverage** — with real credit bodies: (i) classify instantaneous×contrastive — declare the pairing invalid (`IncompatibleCoordinateError`, R3.9 matrix) and/or give contrastive credits an autograd-capable instantaneous path (the gate's instantaneous probe already surfaces it); (ii) add dynamics probes for predictive_settling/spike_integration/diffusion (currently "blocked"-verdict — no probe, so those 54 coordinates can never pass even though diffusion now runs post-R3.1); (iii) decide recurrent×local_goodness under EM: graph-preserving settle vs R3.9 fencing; (iv) rewrite the xfailed axis-cert surrogate-alignment tests to run phases through `run_train_step`'s grad_ctx so FD and pseudo-gradient share one graph; (v) upgrade the M-probe beyond engagement: assert ψ actually changed across `train_step` (non-const) and, for primitives overriding `modulate`, that zeroed-ψ vs stepped-ψ outputs differ (sensitivity)
10. **R5b-A pre-registration** on the widened 30-coordinate manifest (theory-driven, broken-instrument kill criterion). The 8-coordinate R5.1c subspace is superseded; if a hypothesis needs axes still outside the manifest, its repair (not the delta) is the deliverable
...
24. **Axis-cert surrogate-alignment tests need a pipeline-mediated rewrite (2026-08-31)** — `test_local_goodness_surrogate_alignment`/`test_target_inversion_surrogate_alignment` FD the surrogate over manually-computed free/nudged phases (`_run_free_phase`/`_run_nudged_phase`), whose autograd graphs are disconnected from each other, so autograd-based credits can never align with FD there (cos = 0.0). Currently xfailed with reason. Fix: run one `run_train_step` (grad_ctx shared) and FD the same surrogate on its states. Until then these two locks under-verify their credits
