...
if TYPE_CHECKING:
    from bioplausible.core.optimization.config import StrategyOptimizerConfig
...
MODEL_DEFAULTS: dict[str, dict] = {
    ...
    "diff_target_prop": {"hidden_dim": 64, "num_layers": 2, "learning_rate": 0.001, "target_lr": 0.1},
    "predictive_coding_hybrid": {"hidden_dim": 64, "num_layers": 2, "learning_rate": 0.001},
    "eqprop": {"hidden_dim": 64, "num_layers": 2, "learning_rate": 0.001, "beta": 0.1, "max_steps": 20},
}
...
def _construct_model(model_name: str, input_dim: int, output_dim: int, device: torch.device, precision: torch.dtype) -> nn.Module:
...
def _get_data(task_name: str, device: torch.device, precision: torch.dtype, n_samples: int = 256) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for model_name in models:
        for dataset_name in datasets:
            for precision_name in precisions:
                ...
                baseline_model = _construct_model(model_name, input_dim, output_dim, device, dtype)
                
                # Train baseline
                baseline_result = _train_with_strategy(
                    baseline_model, "backprop_plain", "backprop", "plain", "none", "none",
                    x, y, epochs, lr=0.01, precision=precision_name
                )
                baseline_acc = baseline_result.get("accuracy", 0.0) if baseline_result["status"] == "ok" else 0.0
                threshold = baseline_acc * 0.9  # 90% gate
                
                print(f"Model={model_name}, Dataset={dataset_name}, Precision={precision_name}, "
                ...
                for perm_name, grad, upd, constr, fb, compat_models in PERMUTATIONS:
                    ...
                    model = _construct_model(model_name, input_dim, output_dim, device, dtype)
                    
                    result = _train_with_strategy(
                        model, perm_name, grad, upd, constr, fb,
                        x, y, epochs, lr=0.01, precision=precision_name
                    )
                    ...
                    results.append(BenchmarkResult(
                        ...
                        status="ok" if passed else ("error" if result["status"] == "error" else "failed_gate"),
                        error_message=result.get("error_message"),
                    ))
                    
                    status = "✓" if passed else ("✗" if result["status"] != "ok" else "⚠")
                    print(f"  {status} {perm_name}: acc={acc:.4f} "
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("artifacts/strategy_benchmark_report.json"))
