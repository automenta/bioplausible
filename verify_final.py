...
try:
    from eqprop_torch import (
        LoopedMLP, ConvEqProp, EqPropTrainer,
        StandardEqProp, AdaptiveFeedbackAlignment,
        HAS_BIOPLAUSIBLE, enable_tf32
    )
...
try:
    ...
    print(f'✓ LoopedMLP created: {sum(p.numel() for p in model1.parameters()):,} params')
    
    from algorithms import AlgorithmConfig
    config = AlgorithmConfig('eqprop', 784, [256], 10)
    model2 = StandardEqProp(config)
    print(f'✓ StandardEqProp created: {sum(p.numel() for p in model2.parameters()):,} params')
except Exception as e:
